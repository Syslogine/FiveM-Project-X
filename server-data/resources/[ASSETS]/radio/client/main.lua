...
local function handleConnectionEvent(pChannel)
  ...
  if newChannel < 1.0 then
    exports['ethical-voice']:removePlayerFromRadio()
    exports["ethical-voice"]:setVoiceProperty("radioEnabled", false)
  else
    exports["ethical-voice"]:setVoiceProperty("radioEnabled", true)
    exports['ethical-voice']:addPlayerToRadio(newChannel)
  end
end
...
function hasRadio()
    if exports["ethical-inventory"]:hasEnoughOfItem("radio",1,false) then
      return true
    else
      return false
    end
...
RegisterNUICallback('volumeUp', function(data, cb)
  ...
  TriggerEvent('ethical-voice:SetRadioVolume', radioVolume)
