    ...
    if IsNearATM() then
      if ( DoesEntityExist( player ) and not IsEntityDead( player )) then 
          ...
          if ( atmuse ) then 
              ...
              atmuse = false
              local finished = exports["ethical-taskbar"]:taskBar(3000,"Retrieving Card")
              ClearPedTasks(PlayerPedId())
          else
              atmuse = true
        ...
        if ( DoesEntityExist( player ) and not IsEntityDead( player )) then 
            ...
            if ( atmuse ) then 
                ...
                atmuse = false
                local finished = exports["ethical-taskbar"]:taskBar(1000,"Retrieving Card")
                ClearPedTasks(PlayerPedId())
            else
                atmuse = true
                TaskPlayAnim( player, "mp_common", "givetake1_a", 1.0, 1.0, -1, 49, 0, 0, 0, 0 )
...
RegisterNetEvent("ethical-base:addedMoney")
AddEventHandler("ethical-base:addedMoney", function(amt, total)
  TriggerEvent("banking:updateCash", total)
  TriggerEvent("banking:addCash", amt)
end)

RegisterNetEvent("ethical-base:removedMoney")
AddEventHandler("ethical-base:removedMoney", function(amt, total)
  TriggerEvent("banking:updateCash", total)
