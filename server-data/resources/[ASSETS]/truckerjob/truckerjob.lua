  ...
  local vehFront = VehicleInFront()
    if vehFront > 0 then
      ...
      if GetEntityModel(vehFront) == GetHashKey(trailer) then
        local finished = exports["ethical-taskbar"]:taskBar(25000,"Storing Trailer Contents")
...
Citizen.CreateThread(function()
  while true do
    ...
    Wait(1)
    if exports["ethical-base"]:getModule("LocalPlayer"):getVar("job") == "trucker" and existingVeh ~= nil then
...
Citizen.CreateThread(function()
  while true do
    Wait(9000)
    if job ~= 0 and exports["ethical-base"]:getModule("LocalPlayer"):getVar("job") == "trucker"  then
      currentJobPos = returnJobPos(job)
      checkDestruction()
    end
  end
    ...
    if currentJobs[currentJobPos] ~= nil and currentJobs[currentJobPos].JobType == "Shops" then
      ...
      if dist < 20 then
        ...
        if(distance ~= -1 and distance < 5) then
            ...
            TriggerServerEvent("ethical-shops:checkOwner",GetPlayerServerId(t),currentJobs[currentJobPos].shopId)
        else
...
function spawnTruck(i)
      ...
      local finished = exports["ethical-taskbar"]:taskBar(3000,"Checking Vehicle")
      if finished == 100 then
      SetVehicleEngineHealth(spawned_car, 1000.0)
      SetVehicleBodyHealth(spawned_car, 1000.0)
      SetVehiclePetrolTankHealth(spawned_car, 1000.0)
