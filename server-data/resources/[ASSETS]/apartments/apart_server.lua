...
AddEventHandler('house:updatespawns', function(info, hid)
    ...
    local table = info
    local house_id = hid
    local xPlayer = ETHICALCore.GetPlayerFromId(src)
...
AddEventHandler('housing:getGarage', function(house_id, house_model)
    local src = source
    local cid = exports['ethical-core']:GetPlayerCid(src)
...
AddEventHandler('house:givekey', function(house_id,house_model,house_name,target)
    local src = source
    local TargetCID = exports['ethical-core']:GetPlayerCid(tonumber(target))
