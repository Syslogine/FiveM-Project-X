...
Citizen.CreateThread(function()
    while true do
        ...
        if statusS == true then
            TriggerServerEvent("ethical-spawn:inspawn", true)
        end
    end
end)
...
function Save(save, close)
    ...
    if save then
        ...
        if not startingMenu or passedClothing then
            ...
            TriggerEvent("ethical-spawn:finishedClothing","Finished")
        ...
        TriggerEvent("ethical-spawn:finishedClothing","Old")
        LoadPed(oldPed)
    end
    ...
    TriggerEvent('ethical-weapons:getAmmo')
...
AddEventHandler("ethical-base:initialSpawnModelLoaded", function()
...
AddEventHandler('raid_clothes:ListOutfits', function(skincheck)
  ...
  TriggerEvent('ethical-ui:ShowUI', 'show', menuData)
end)
...
AddEventHandler("ethical-polyzone:enter", function(zone, data)
    ...
    if currentZone then
        exports["ethical-interaction"]:showInteraction("[M] ".. currentZone.text)
