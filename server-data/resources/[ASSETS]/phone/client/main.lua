...
RegisterCommand("cid", function(source, args)
  ...
  TriggerEvent("DoLongHudText", "Here's your CID:"..cid)
end)
...
AddEventHandler("phone:error", function()
      SendNUIMessage({
        openSection = "error",
        textmessage = "<b>Network Error</b> <br><br> Please contact support if this error persists, thank you for using Imagination Phone Services.",
      })   
end)
...
function findVehFromPlateAndLocate(plate)
  ...
  for ind, value in pairs(vehicles) do
    ...
    if vehPlate == plate then
      ...
      if coordlocation == nil or confirmation == 'null' then
        TriggerEvent('DoLongHudText', 'Your vehicle is in your garage or does not have updated coords!', 2)
      else
        SetNewWaypoint(json.decode(coordlocation)["x"], json.decode(coordlocation)["y"])
      end
    end
  end
end
...
function hasDecrypt2()
    if exports["ethical-inventory"]:hasEnoughOfItem("vpnxj",1,false) and not exports["isPed"]:isPed("disabled") then
      return true
    else
      return false
    end
...
function hasTrucker()
    if exports["ethical-base"]:getModule("LocalPlayer"):getVar("job") == "trucker"  then
      return true
    else
      return false
    end
...
function hasDecrypt()
    if exports["ethical-inventory"]:hasEnoughOfItem("decrypterenzo",1,false) or exports["ethical-inventory"]:hasEnoughOfItem("decryptersess",1,false) or exports["ethical-inventory"]:hasEnoughOfItem("decrypterfv2",1,false) and not exports["isPed"]:isPed("disabled") or exports["ethical-inventory"]:hasEnoughOfItem(80,1,false) and not exports["isPed"]:isPed("disabled") then
      return true
    else
      return false
    end
...
function hasDevice()
    if exports["ethical-inventory"]:hasEnoughOfItem("mk2usbdevice",1,false) and not exports["isPed"]:isPed("disabled") then
      return true
    else
      return false
    end
...
function hasPhone()
    if
      (
      (exports["ethical-inventory"]:hasEnoughOfItem("mobilephone",1,false) or 
      exports["ethical-inventory"]:hasEnoughOfItem("stoleniphone",1,false) or 
      exports["ethical-inventory"]:hasEnoughOfItem("stolens8",1,false) or
      exports["ethical-inventory"]:hasEnoughOfItem("stolennokia",1,false) or
      exports["ethical-inventory"]:hasEnoughOfItem("stolenpixel3",1,false) or
      exports["ethical-inventory"]:hasEnoughOfItem("boomerphone",1,false))
...
function hasRadio()
    if exports["ethical-inventory"]:hasEnoughOfItem("radio",1,false) and not exports["isPed"]:isPed("disabled") then
      return true
    else
      return false
    end
...
function openGui()
  ...
  if hasPhone() then
    TriggerServerEvent('white:setMetaHere')
    GiveWeaponToPed(PlayerPedId(), 0xA2719263, 0, 0, 1)
    guiEnabled = true
      SetNuiFocus(false,false)
      SetNuiFocus(true,true)
...
AddEventHandler('phone:hangup', function()
  if isNotInCall then return end
    t("Call Hangup")
    endCall()
end)
...
AddEventHandler('phone:addToCall', function(voipchannel)
  exports['ethical-voice']:addPlayerToCall(tonumber(voipchannel))
end)
...
function receivingCall(callFrom)
  ...
  while (callTimer > 0 and callStatus == isReceivingCall) do
    if hasPhone() then
      Citizen.Wait(1)
      TriggerEvent("DoLongHudText","Call from: " .. callFrom .. " use your hotkeys to answer or hangup!", 1)
...
AddEventHandler('phone:removefromToko', function(playerRadioChannel)
  exports['ethical-voice']:removePlayerFromCall()
end)
...
AddEventHandler('sendMessagePhoneN', function(phonenumberlol)
  TriggerServerEvent('message:tome', tonumber(phonenumberlol))

  local closestPlayer, closestDistance = GetClosestPlayer()
	if closestPlayer ~= -1 and closestDistance <= 5.0 then
    TriggerServerEvent('message:inDistanceZone', GetPlayerServerId(closestPlayer), tonumber(phonenumberlol))
  else    
  end
end)
...
RegisterCommand('answerd',function()
  TriggerEvent("phone:answercall")
end)
...
RegisterCommand('hangd', function()
  TriggerEvent("phone:hangup")
end)
...
Citizen.CreateThread(function()
  RegisterKeyMapping("answer", "Answer Player's Calls", "keyboard", "I") 
  RegisterCommand('answer', answercall, false)
end)

Citizen.CreateThread(function()
  RegisterKeyMapping("hangup", "Hangup Player's Calls", "keyboard", "H") 
  RegisterCommand('hangup', hangupcall, false)
end)
