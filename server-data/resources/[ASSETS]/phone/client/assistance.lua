...
Citizen.CreateThread(function()
  ...
  while true do
    ...
    if nearEntertainment() and exports["ethical-base"]:getModule("LocalPlayer"):getVar("job") == "entertainer" then
    ...
    elseif exports["ethical-base"]:getModule("LocalPlayer"):getVar("job") == "news" then
      local dist = 0
      if lastBlip.x then
      ...
      if exports["ethical-base"]:getModule("LocalPlayer"):getVar("job") == "entertainer" then
        Citizen.Wait(300000)
      end
    end
      ...
      if (key ~= nil and item ~= nil) then
        ...
        if #(vector2(pos.x, pos.y) - vector2(item.pos.x, item.pos.y)) < 50.0 then
          if item.jobType == "ems" then
            if exports["ethical-base"]:getModule("LocalPlayer"):getVar("job") == "ems" then
          ...
          elseif item.jobType == "news" then 
            if exports["ethical-base"]:getModule("LocalPlayer"):getVar("job") == "news" then
              TriggerServerEvent('phone:assistRemove', item.id, item.jobType) -- Send message of clear to others
              clearBlip(item)
              if GetTimeDifference(curTime, item.timestamp) > 2000 then
                if canPay then
