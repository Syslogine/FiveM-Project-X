...
RegisterNetEvent("returncar")
AddEventHandler("returncar",function()
local src = source
...
AddEventHandler('police:showPH', function()
	local src = source
	local player = exports["ethical-base"]:getModule("Player"):GetUser(src)
...
AddEventHandler('police:jailGranted', function(args)
	...
	if not pdunit:getVar("job") == "police" then
		...
		exports["ethical-log"]:AddLog("Exploiter", pdunit, "User Attempted to jail player while not on pd", {target = playerName, reason = reason, cid = cid, time = tonumber(args[2]), src= src})
		DropPlayer(src, "")
	...
	exports["ethical-base"]:getModule("JobManager"):SetJob(target, "unemployed", true)
...
AddEventHandler("police:deletecrimes", function(target)
	...
	if not pdunit:getVar("job" =="police") then
		...
		exports["ethical-log"]:AddLog("Exploiter", pdunit, "User Attempted delete crimes while not on duty", {target = target , src = src})
		DropPlayer(src, "")
...
AddEventHandler('police:updateLicenses', function(targetlicense, status, license)
	...
	local targetuser = exports["ethical-base"]:getModule("Player"):GetUser(targetlicense)
	local char = targetuser:getVar("character").id
	local Lname = targetuser:getVar("character").last_name
	print(targetlicense)
	print(char)
	print(status)
	print(license)
    if status == 1 then
		-- add license
		TriggerClientEvent('DoLongHudText',src, 'You granted Mr/Ms. '..Lname..' '..license..' License.')
		GrantLicense(targetlicense,char,status,license)
	else
		-- remove license
		TriggerClientEvent('DoLongHudText',src, 'You revoked Mr/Ms. '..Lname..' '..license..' License.')
		removeLicense(targetlicense,char,status,license)
    end
end)

function GrantLicense(src,cid,status,license)
...
AddEventHandler('police:SeizeCash', function(target)
	...
	local user = exports["ethical-base"]:getModule("Player"):GetUser(src)
    local characterId = user:getVar("character").id
	local userz = exports["ethical-base"]:getModule("Player"):GetUser(target)
...
RegisterCommand('fine', function(source, args)
	...
	local amount = args[2]
	TriggerClientEvent('bank:fineplayer', source, target, amount)
...
AddEventHandler('police:setServerMeta', function(health, armor, thirst, hungry)
    local src = source
    local user = exports["ethical-base"]:getModule("Player"):GetUser(src)

    -- Check if user exists before proceeding
    if not user then
        print("No user found.") -- Optional: Log or handle the error as needed
        return
    end

    local char = user:getCurrentCharacter()

    -- Check if char is a valid character object
    if not char or type(char) ~= "table" then
        print("No character found or character data is invalid.") -- Optional: Log or handle as needed
        return
    end

    local q = [[UPDATE characters SET metaData = @meta WHERE id = @cid;]]
    local v = {
        ["meta"] = json.encode({["health"] = health, ["armour"] = armor, ["thirst"] = thirst, ["hunger"] = hungry}),
        ["cid"] = char.id
    }

    exports.oxmysql:execute(q, v, function()
        -- Callback code here, if needed
    end)
end)
...
-- 	local commands = exports["ethical-base"]:getModule("Commands")
...
function jailUser(player,reason,cid,time,src)
	local jailer = exports["ethical-base"]:getModule("Player"):GetUser(src)
	...
	if not jailer:getVar("job") == "police" and not jailer:getVar("job") == "doc" then
		...
		exports["ethical-log"]:AddLog("Exploiter", jailer, "User attempted to jail while not on pd; Reason: " ..tostring(reason), {target = player, reason = reason, cid = cid, time = time})
		DropPlayer(src, "")
		return
	end
	...
	TriggerEvent("ethical-police:playerJailed", character)
...
AddEventHandler("911", function(args)
	...
	exports["ethical-log"]:AddLog("911 Chat", user, caller .. ": " .. message, {})
	...
	local users = exports["ethical-base"]:getModule("Player"):GetUsers()

	local emergencyPlayers = {}

	for k,v in pairs(users) do
		local user = exports["ethical-base"]:getModule("Player"):GetUser(v)
		local job = user:getVar("job")
...
RegisterServerEvent("police:showBadge")
AddEventHandler("police:showBadge", function()
	local src = source
	local users = exports["ethical-base"]:getModule("Player"):GetUser(src)
	...
	if job == "police" then
		...
		local array = { user = {
			...
			dateofbirth = char.dob,
			sex = sex,
			pstate = police_state
			}
		}
		TriggerClientEvent('badge:open', src, array, "")
	end
end)
...
AddEventHandler("911r", function(args)
	...
	local caller = tostring(char.first_name) .. " " .. tostring(char.last_name)
	exports["ethical-log"]:AddLog("911r Chat", user, caller .. ": " .. message)
	...
	exports["ethical-log"]:AddLog("911r Chat", user, char.first_name .. " " .. char.last_name .. ": ".. message, {target = target})
end)
...
function selectProperties(src, callback)
	local user = exports['ethical-base']:getModule("Player"):GetUser(src)
	local character = user:getCurrentCharacter()
