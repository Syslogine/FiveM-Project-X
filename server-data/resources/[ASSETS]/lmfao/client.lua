		...
		--	end
			...
			if GetDistanceBetweenCoords(PlayerPos, 2221.7973632813,5577.0966796875,53.844783782959, true) <= 10 then		
				...
				if JustDriedWeed or JustPackedWeed then
					TriggerEvent("DoLongHudText","You are too tired from a different task, either wait 8 minutes or get lost")
				else
			...
			if GetDistanceBetweenCoords(PlayerPos, -1171.6, -1572.13, 4.67, true) <= 1 then						
				-- selling bud
				local smallbud = exports["ethical-inventory"]:hasEnoughOfItem("smallbud",5,false)
				if smallbud then
					TriggerEvent("inventory:removeItem", "smallbud", 5)
					TriggerServerEvent( 'mission:completed', 450 )
				else
			...
			if GetDistanceBetweenCoords(PlayerPos, 3818.36, 4441.23, 2.81, true) <= 5 then						
				...
				if JustPickedWeed or JustPackedWeed then
					...
					local wetbud = exports["ethical-inventory"]:hasEnoughOfItem("wetbud",5,false)
					if wetbud then

						local finished = exports["ethical-taskbar"]:taskBar(10000,"Drying Weed",false,false,playerVeh)
			...
			if GetDistanceBetweenCoords(PlayerPos, 3825.82, 4441.26, 2.81, true) <= 5 then						
				...
				if JustPickedWeed or JustDriedWeed then
					...
					local wetbud = exports["ethical-inventory"]:hasEnoughOfItem("driedbud",5,false)
					if wetbud then

						local finished = exports["ethical-taskbar"]:taskBar(10000,"Packing Weed",false,false,playerVeh)
						if (finished == 100) then
			...
			if GetDistanceBetweenCoords(PlayerPos, 5.87, -1601.48, 29.3, true) <= 5 then						
				...
				local smallbud = exports["ethical-inventory"]:hasEnoughOfItem("freshmeat",5,false)
				if smallbud then
