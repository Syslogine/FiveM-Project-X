...
Citizen.CreateThread(function()
	exports["ethical-polyzone"]:AddBoxZone("mech_stash", vector3(963.32, -979.36, 39.5), 1.8, 2.4, {
			heading=315,
			minZ=30.44,
			maxZ=45.04
	  })
...
AddEventHandler('fakeplate:change', function()
	...
	if targetVehicle ~= nil  and targetVehicle ~= 0 then
		...
		local finished = exports["ethical-taskbar"]:taskBar(20000,"Changing plates", true)
...
AddEventHandler('towgarage:craft', function(res,amount)
	...
	if rank >= 4 or rank2 > 1 then -- Eddie wants rank 4 for crafting
		...
		local finished = exports["ethical-taskbar"]:taskBar(1000,"Completing Task",false,true)

		if finished ~= 100 then
			return
		end
		if res == 1 and rank > 1 then
			local hasitems = exports["ethical-inventory"]:hasEnoughOfItem("electronics",100*amount,true)
			if hasitems then
		...
		elseif res == 2 then
			...
			local hasitems = exports["ethical-inventory"]:hasEnoughOfItem("copper",3*amount,true)
			if hasitems then
		...
		elseif res == 3 then
			...
			local hasitems = exports["ethical-inventory"]:hasEnoughOfItem("aluminium",2*amount,true)
			if hasitems then
		...
		elseif res == 4 then
			...
			local hasitems = exports["ethical-inventory"]:hasEnoughOfItem("steel",2*amount,true)
			if hasitems then
				TriggerEvent("inventory:removeItem","steel", 2*amount)
				TriggerEvent("player:receiveItem","lockpick",amount)
			end
		elseif res == 5 then
			local hasitems = exports["ethical-inventory"]:hasEnoughOfItem("electronics",3*amount,true)
			if hasitems then
		...
		elseif res == 6 and rank > 1 then
			local hasitems = exports["ethical-inventory"]:hasEnoughOfItem("rubber",10*amount,true)
			if hasitems then
				...
				if IsControlJustPressed(1, Controlkey["generalUseThird"][1]) and garageDropDistance < 1.0 then
					...
					if (i == 3 and drank > 0) or (i == 2 and rank > 0) or (i == 1 and crank > 0) or (i == 4 and zrank > 0) or (i == 5 and lrank > 0) then
						...
						for x = 1, #itemvalues do
							local itemid = itemvalues[x]["itemid"]
							local itemname = itemvalues[x]["itemname"]
							local itemcount = exports["ethical-inventory"]:getQuantity(itemvalues[x]["name"])

							if itemcount > 0 then
								local finished = exports["ethical-taskbar"]:taskBar(2000,"Storing " .. itemname)
								if finished == 100 then
...
AddEventHandler('towgarage:TriggerRepairs', function(degname,amount)
	...
	if not notfucked then
		...
		local finished = exports["ethical-taskbar"]:taskBar(15000,"Repairing")
				...
				if #(offsetMark[rand] - GetEntityCoords(LocalPed())) < 0.9 then
					...
					if IsControlJustPressed(1, Keys["E"]) then
						if inUse then
							...
							local finished = exports["ethical-taskbar"]:taskBar(5000,"Repairing")
...
local function listenForKeypress(zoneName, zoneData)
    listening = true
    Citizen.CreateThread(function()
       
        -- local currentCash = RPC.execute("ethical-financials:getCurrentCash")
        while listening do
...
AddEventHandler("ethical-polyzone:enter", function(zone, data)
    ...
    if currentZone then
        exports["ethical-interaction"]:showInteraction("[E] To open Mech stash", "information")
        -- print("ZONE AND DATA", zone, currentZone, json.encode(data))
        listenForKeypress(zone, currentZone)
    end
end)
