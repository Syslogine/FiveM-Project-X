...
local bollards = {
    officer_sign_in = {
        text = "Duty Action",
        displayName = "Duty Actions"
    }
}

Citizen.CreateThread(function()
	exports["ethical-polyzone"]:AddBoxZone("officer_sign_in", vector3(441.83, -982.05, 30.69), 0.5, 0.35, {
		heading=12,
		minZ=30.79,
		maxZ=30.84
	  })
...
AddEventHandler("ethical-polyzone:exit", function(zone)
	if zone == "officer_sign_in" then
		print("SECCCSSSSSSS111111111111111111111")
    end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		TriggerServerEvent("ethical-base:updatecoords",coords.x,coords.y,coords.z)
		Wait(13000)
	end
end)
...
AddEventHandler('police:duty', function()
    exports['menu']:SetTitle("Police Duty")
    exports['menu']:AddButton("On Duty" , "On Duty Officer" ,'police:onduty' ,'Lets duty now' , 'menuone')
    exports['menu']:AddButton("Off Duty" , "Off Duty Officer" ,'police:offduty' ,'Lets duty now' , 'menuone')
    exports['menu']:SubMenu("Duty Sign In" , "You can sign in and out here" , "menuone" )
   
end)

RegisterNetEvent('police:onduty')
AddEventHandler('police:onduty', function()
    TriggerEvent('event:control:policeGarage', 1)
end)

RegisterNetEvent('police:offduty')
AddEventHandler('police:offduty', function()
    TriggerEvent('event:control:policeGarage', 2)
end)
			...
			if dstCheck < 3 then
				...
				if dstCheck < 1 then
					...
					if IsControlJustReleased(1, 38) then
						...
						DoScreenFadeOut(500)
						Wait(1000)
						TriggerEvent("ethical-base:clearStates")
						exports["ethical-base"]:getModule("SpawnManager"):Initialize()
						Wait(1000)
					end
				end
			end
	...
	while true do 
		...
		if recentsearch == 0 then
			...
			if endDistance < 0.7 or endDistance2 < 0.7 then
				...
				currentWeapons = exports["ethical-inventory"]:GetCurrentWeapons() 
...
AddEventHandler("police:rob", function()
	...
	if not exports["isPed"]:isPed("dead") then
		...
		local finished = exports["ethical-taskbar"]:taskBar(3500,"Robbing",false,true)	
...
AddEventHandler( 'KneelHU', function()
	...
	if ( DoesEntityExist( player ) and not IsEntityDead( player )) then 
		...
		local finished = exports["ethical-taskbar"]:taskBar(2500,"Surrendering")					
...
AddEventHandler('police:wshotgun', function()
    ...
    if targetVehicle ~= 0 then
        ...
        if dist < 1.0 then
			...
			local finished = exports["ethical-taskbar"]:taskBar(2000,"Unracking Shotgun")
		  	if finished == 100 then
    ...
    if targetVehicle ~= 0 then
        ...
        if dist < 1.0 then
			...
			SetVehicleDoorOpen(targetVehicle, 0, 1, 1)
	  		loadAnimDict('anim@narcotics@trash')
			TaskPlayAnim(PlayerPedId(),'anim@narcotics@trash', 'drop_front',0.9, -8, 1900, 49, 3.0, 0, 0, 0)
			local finished = exports["ethical-taskbar"]:taskBar(2000,"Unracking Rifle")
		  	if finished == 100 then
...
AddEventHandler('police:woxy', function()
	...
	if vehFront > 0 then
		...
		local finished = exports["ethical-taskbar"]:taskBar(4000,"Grabbing Scuba Gear")
	  	if finished == 100 then
	  		loadAnimDict('anim@narcotics@trash')
    		TaskPlayAnim(PlayerPedId(),'anim@narcotics@trash', 'drop_front',0.9, -8, 1900, 49, 3.0, 0, 0, 0)	  		
...
AddEventHandler('giveRadioToPed', function()
	...
	if (not exports["ethical-inventory"]:hasEnoughOfItem("radio",1,false) ) then
...
AddEventHandler('swatcopLoadout', function()
    ...
    if targetVehicle ~= 0 then
        ...
        if dist < 1.0 then
			...
			local finished = exports["ethical-taskbar"]:taskBar(2500,"Changing to Swat")
	  		if finished == 100 then
				ChangeToSkinNoUpdatePolice("s_m_y_swat_01")		
			end
			ClearPedTasks(PlayerPedId())
...
AddEventHandler('police:giveweaponlicense', function(t)
	t, distance = GetClosestPlayer()
	if(distance ~= -1 and distance < 5) then
		TriggerServerEvent("police:updateLicenses", GetPlayerServerId(t),1,"weapon")
	else
		TriggerEvent("DoLongHudText", "No player near you (maybe get closer)!",2)
	end
end)
...
AddEventHandler('police:removeweaponlicense', function(t)
	t, distance = GetClosestPlayer()
	if(distance ~= -1 and distance < 5) then
		TriggerServerEvent("police:updateLicenses", GetPlayerServerId(t),0,"weapon")
	else
		TriggerEvent("DoLongHudText", "No player near you (maybe get closer)!",2)
	end
end)
...
AddEventHandler('police:gsr', function(t)
	...
	local finished = exports["ethical-taskbar"]:taskBar(15000,"GSR Testing")
    if finished == 100 then
		t, distance = GetClosestPlayer()
		if(distance ~= -1 and distance < 7) then
...
AddEventHandler('police:getArrested2', function(cuffer)
	...
	local finished = 0
	if not isDead then
		finished = exports["ethical-taskbarskill"]:taskBar(1200,7)
	end
	...
	if t ~= nil and t ~= -1 then
		if(distance ~= -1 and distance < 5) then
			...
			if myjob ~= "ems" and myjob ~= "doctor" then
				local bandages = exports["ethical-inventory"]:getQuantity("bandage")
				if bandages == 0 then
					return
				else
...
AddEventHandler("ems:stomachpump", function()
	...
	if t ~= nil and t ~= -1 then
		if(distance ~= -1 and distance < 5) then
			local finished = exports["ethical-taskbar"]:taskBar(10000,"Inserting stomach pump 🤢", false, true)
...
AddEventHandler('FlipVehicle', function()
	local finished = exports["ethical-taskbar"]:taskBar(5000,"Flipping Vehicle Over",false,true)	
...
function deleteVeh1(ent)

	SetVehicleHasBeenOwnedByPlayer(ent, true)
	NetworkRequestControlOfEntity(ent)
	local finished = exports["ethical-taskbar"]:taskBar(1000,"Returning Vehicle",false,true)	
...
function deleteVeh(ent)
	...
	SetVehicleHasBeenOwnedByPlayer(ent, true)
	NetworkRequestControlOfEntity(ent)
	local finished = exports["ethical-taskbar"]:taskBar(1000,"Impounding",false,true)	
	Citizen.InvokeNative(0xEA386986E786A54F, Citizen.PointerValueIntInitialized(ent))
	DeleteEntity(ent)
	DeleteVehicle(ent)
	SetEntityAsNoLongerNeeded(ent)
...
RegisterNetEvent('returnVehicle')
AddEventHandler('returnVehicle', function()
   ...
   -- targetVehicle = getVehicleInDirection(coordA, coordB)
	...
	deleteVeh1(targetVehicle)
	TriggerServerEvent("returncar")
	Citizen.Wait(1000)
	TriggerEvent("DoLongHudText","Successfully returned vehicle!",1)
...
local function serviceVehicle(arg, livery, isEmsWhiteListed, cb)
	...
	Citizen.CreateThread(function()
		...
		TriggerEvent("ethical-admin:runSpawnCommand",selectedSkin, livery)
	end)
end
...
if isCop then
	...
	if cmd == "fix" then
		local finished = exports["ethical-taskbar"]:taskBar(3500,"Completing Task",false,true)	
...
RegisterNetEvent("ethical-gov:police:showBadge")
AddEventHandler("ethical-gov:police:showBadge", function(pSource, pInventoryData)
    local veh = GetVehiclePedIsIn(PlayerPedId(), false)
    local isInCar = veh ~= 0 and veh ~= nil
    -- if GetPlayerServerId(PlayerId()) ~= pSource then
    --   Citizen.CreateThread(function()
    --     Citizen.Wait(isInCar and 1000 or 4500)
    --     exports["ethical-ui"]:openApplication("badge", {
...
RegisterNetEvent('badge:close')
AddEventHandler('badge:close', function()
	SendNUIMessage({
		action = "close"
	})
	open = false
...
RegisterNetEvent('police:switchChar')
AddEventHandler('police:switchChar', function(data)
	if data == "newChar" then
		DoScreenFadeIn(100)
	end
	isCop = false
	TriggerEvent("inhouse",false)
    TransitionToBlurred(1500)
    DoScreenFadeOut(1500)
	TaskLookAtCoord(PlayerPedId(), vector3(-197.75,-1002.46,28.15 ),-1, 0, 2)
    TriggerEvent("ethical-base:clearStates")
    exports["ethical-base"]:getModule("SpawnManager"):Initialize()
    Wait(500)
...
local function TaskBar(pLength, pName, pRunCheck, pHidden)
    ...
    Citizen.CreateThread(function ()
        if pName then
            exports['ethical-taskbar']:taskBar(pLength, pName, pRunCheck, true, nil, false, function (result)
                p:resolve(result)
            end)
        else
...
function AnimationTask:abort()
    ...
    exports['ethical-taskbar']:taskCancel();
end

RegisterCommand("search", function(source, args)
	TriggerEvent("police:checkInventory")
end)

-- RegisterCommand("fine", function(source, args)
-- 	local user = args[1]
-- 	local amount = args[2]
-- 	TriggerServerEvent("police:payfines", user, amount)
-- 	print("Bill", args[1], args[2])
-- 	print('Player',GetPlayerServerId(args[1]))
-- 	TriggerServerEvent("police:payfines", user, amount)
-- 	-- t, distance, closestPed = GetClosestPlayer()
