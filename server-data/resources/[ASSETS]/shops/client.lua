...
function setShopBlip()
	...
	created_ped = CreatePed(0, modelHash , 457.52, -991.13, 24.70,true)
	FreezeEntityPosition(created_ped, true)
	SetEntityHeading(created_ped,  85.28)
	SetEntityInvincible(created_ped, true)
	SetBlockingOfNonTemporaryEvents(created_ped, true)
	...
	created_ped2 = CreatePed(0, modelHash , 328.13, -578.01, 27.8,true)
	FreezeEntityPosition(created_ped2, true)
	SetEntityHeading(created_ped2,  333.91)
	SetEntityInvincible(created_ped2, true)
	SetBlockingOfNonTemporaryEvents(created_ped2, true)
	TaskStartScenarioInPlace(created_ped2, "WORLD_HUMAN_CLIPBOARD", 0, true)
	...
	modelHash = GetHashKey("a_f_y_business_03")
	RequestModel(modelHash)
	
	while not HasModelLoaded(modelHash) do
	 	Wait(1)
	end
	created_ped = CreatePed(0, modelHash , 172.88, -26.55, 67.3,true)
	FreezeEntityPosition(created_ped, true)
	SetEntityHeading(created_ped,  160.97)
	SetEntityInvincible(created_ped, true)
	SetBlockingOfNonTemporaryEvents(created_ped, true)
	TaskStartScenarioInPlace(created_ped, "WORLD_HUMAN_CLIPBOARD", 0, true)
	...
	created_ped1 = CreatePed(0, modelHash , 343.29, -594.76, 73.16,true)
	FreezeEntityPosition(created_ped1, true)
	SetEntityHeading(created_ped1,  319.36)
...
AddEventHandler('pmeth:general', function()
	local finished = exports["ethical-taskbar"]:taskBar(60000,"Searching...")
...
AddEventHandler('slushy:general', function()
	local finished = exports["ethical-taskbar"]:taskBar(60000,"Making a god slushy...")
	if (finished == 100) then
...
-- 	local finished = exports["ethical-taskbar"]:taskBar(60000,"What dis????")
-- 	if (finished == 100) then
