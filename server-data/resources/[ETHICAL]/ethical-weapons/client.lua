...
RegisterNetEvent('ethical-weapons:destroyGroundWeapon')
AddEventHandler('ethical-weapons:destroyGroundWeapon', function()
	DeleteEntity(weapon)
end)

RegisterNetEvent('ethical-weapons:allowDrop')
AddEventHandler('ethical-weapons:allowDrop', function()
	canDrop = true
end)

RegisterNetEvent('ethical-weapons:dropweapon')
AddEventHandler('ethical-weapons:dropweapon', function(AmountOfCurrentWeapon,weaponModel,rowID)
    ...
    canDrop = false
	TriggerServerEvent("ethical-weapons:dropWeapon",{pos.x,pos.y,pos.z+0.75},name,rowID)
...
RegisterNetEvent('ethical-weapons:doHandOver')
AddEventHandler('ethical-weapons:doHandOver', function(weapon)
  --GiveWeaponToPed(PlayerPedId(), tonumber(weapon), 0, 0, false)
  TriggerEvent("attachWeapons")
end)

RegisterNetEvent('ethical-weapons:dropWeaponSearch')
AddEventHandler('ethical-weapons:dropWeaponSearch', function(weapon)
	--RemoveWeaponFromPed(PlayerPedId(),tonumber(weapon))
  TriggerEvent("attachWeapons")
end)


RegisterNetEvent('ethical-weapons:giveweapon')
AddEventHandler('ethical-weapons:giveweapon', function(toPlayer,AmountOfCurrentWeapon,weaponModel,rowID)
	...
	if not IsPedInAnyVehicle(PlayerPedId(),true) then
		...
		if not IsNearPlayer(toPlayer) then TriggerEvent('chatMessagess', "", 1, "^1You are not near this player!") return end
		...
		if Vdist2(targetPos, userCoords) > 15.0 then
			TriggerEvent('chatMessagess', "", 1, "^1You are not near this player!")
			return
		end
		...
		if (playing ~= false) then
			...
			TriggerServerEvent("ethical-weapons:giveweapon", toPlayer, name,rowID)
			Wait(1300)
			TriggerServerEvent("weaponshop:removeSingleWeapon",rowID)
			TriggerEvent("attachWeapons")
		else
			TriggerEvent('chatMessagess', "", 1, "^1This player is not online!")
		end
...
AddEventHandler('attachWeapons', function()
	...
	local currentWeapons = exports["ethical-inventory"]:GetCurrentWeapons() 
	weapcount = 1
	local curw = GetSelectedPedWeapon(PlayerPedId())
...
AddEventHandler('DisplayWeapons', function(results) 
	...
	TriggerEvent('chatMessagess', 'SEARCH - WEAPONS', 1, " TOTAL: (" .. #results .. ")")
	...
	for k,v in ipairs(results) do
		...
		if tostring( hashGunToText[curCheck] ) ~= nil then
			...
			if v.originalCID == nil or v.originalCID == 0 then
				TriggerEvent('chatMessagess', 'SEARCH - WEAPONS', 1, hashGunToText[curCheck] .. " - (Unknown) ".." - Cartridge: "..cartridge)
			else
				if hashGunToText[curCheck] == nil then
					TriggerEvent('chatMessagess', 'SEARCH - WEAPONS', 1, "Error: Unknown Weapon - (" .. v.originalCID .. ")".." - Cartridge: "..cartridge)
				else
					TriggerEvent('chatMessagess', 'SEARCH - WEAPONS', 1, hashGunToText[curCheck] .. " - (" .. v.originalCID .. ")".." - Cartridge: "..cartridge)
				end				
			end	
		else
			TriggerEvent('chatMessagess', 'SEARCH - WEAPONS', 1, v.weapon_model .. " - (" .. v.originalCID .. ")".." Cartridge: "..cartridge)
		end
		if isMelee then
			for _,j in ipairs(metaData.dna) do
				TriggerEvent('chatMessagess', 'Evidence - WEAPONS', 1, hashGunToText[curCheck] .." - Dna: "..j)
			end
		end
