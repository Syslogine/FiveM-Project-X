local stashes = {}
local instash = 0
RegisterNetEvent('ethical-stash:setInitialState')
AddEventHandler('ethical-stash:setInitialState', function(stashesp)
    print('loaded')
    stashes = stashesp
...
CreateThread(function()
    TriggerServerEvent("ethical-stash:fetchInitialState")
end)
...
RegisterNetEvent('ethical-stash:getnearstash')
AddEventHandler('ethical-stash:getnearstash', function()
    local closestDoorDistance, closestDoorId = 9999.9, -1
    local currentPos = GetEntityCoords(PlayerPedId())
    for id, handle in pairs(stashes) do
        local currentDoorDistance = #(stashes[id].StashEntry - currentPos)
...
RegisterNUICallback('complete', function(data, cb)
    ...
    cb('ok');
    TriggerEvent('ethical-stash:getnearstash')
end)
