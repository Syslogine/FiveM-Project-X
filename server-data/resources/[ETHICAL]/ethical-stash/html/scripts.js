...
function pinFailure() {
  $.post('http://ethical-stash/failure', JSON.stringify({pinResult: false}));
  gameOver = true;
  $('#modal').fadeIn();
  ++failedAttempts;
  if (failedAttempts >= 3)
...
function pinSuccess(pElevated) {
  $.post('http://ethical-stash/complete', JSON.stringify({pinResult: true, owner: pElevated}));
...
document.onkeyup = function (data) {
  if (data.which == 27 ) {
    $.post('http://ethical-stash/close', JSON.stringify({pinResult: false}));
  }
};
