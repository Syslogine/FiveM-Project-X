...
local pillStore =  { ['x'] = -778.49670410156, ['y'] = -600.52746582031, ['z'] = 30.274047851562, ['h'] = 184.25196838379, ['info'] = ' oxy' }

local pillWorker = { ['x'] = -778.77362060547, ['y'] = -596.90112304688, ['z'] = 4.072509765625, ['h'] = 178.58265686035, ['info'] = ' lol' }
...
function buildDrugShop()
	...
	SetEntityCoords(PlayerPedId(), -779.34063720703,-605.07690429688,4.072509765625)
	Citizen.Wait(500)
...
function CreateDrugStorePed()
	...
	drugStorePed = CreatePed(pedType, hashKey, pillWorker["x"],pillWorker["y"],pillWorker["z"], 178.58265686035, 1, 1)
...
local carspawns = {
	[1] =  { ['x'] = -767.34063720703, ['y'] = -593.32745361328, ['z'] = 30.122314453125, ['h'] = 0.0, ['info'] = ' park 1' },
	[2] =  { ['x'] = -771.41540527344, ['y'] = -593.02416992188, ['z'] = 30.122314453125, ['h'] = 0.0, ['info'] = ' park 2' },
	[3] =  { ['x'] = -775.39782714844, ['y'] = -593.20880126953, ['z'] = 30.122314453125, ['h'] = 0.0, ['info'] = ' park 3' },
	[4] =  { ['x'] = -778.98461914062, ['y'] = -593.57800292969, ['z'] = 30.122314453125, ['h'] = 0.0, ['info'] = ' park 4' },
	[5] =  { ['x'] = -786.10546875, ['y'] = -593.57800292969, ['z'] = 30.122314453125, ['h'] = 348.6614074707, ['info'] = ' park 5' },
	[6] =  { ['x'] = -792.23736572266, ['y'] = -590.45275878906, ['z'] = 30.122314453125, ['h'] = 328.81890869141, ['info'] = ' park 6' },
	[7] =  { ['x'] = -795.44177246094, ['y'] = -588.31646728516, ['z'] = 30.122314453125, ['h'] = 331.65353393555, ['info'] = ' park 7' },
	[8] =  { ['x'] = -798.65936279297, ['y'] = -586.087890625, ['z'] = 30.122314453125, ['h'] = 323.14959716797, ['info'] = ' park 8' },
}
...
function HasStolenGoods()
	if OxyRun then
		...
		for i = 84, 97 do
			local itemcount = exports["ethical-inventory"]:hasEnoughOfItem(stolenGoodsTable[i],1,false)
			if itemcount then
...
local bandprice = 15000 -- per 20 = 750ea
local rollcashprice = 850 -- per 30 = 60ea
	...
	if DoesEntityExist(deliveryPed) and not IsEntityDead(deliveryPed) then
		...
		if HasStolenGoods() then
			...
			if OxyRun then
				...
				if exports["ethical-inventory"]:hasEnoughOfItem("inkedmoneybag",1,false) then     
					-- RepPlus()
					TriggerEvent("inventory:removeItem","inkedmoneybag", 1)   
					cashPayment = cashPayment + 85000  

		        elseif exports["ethical-inventory"]:hasEnoughOfItem("rollcash",10,false) then     
		        ...
		        elseif exports["ethical-inventory"]:hasEnoughOfItem("markedbills",20,false) then     
		            TriggerEvent("inventory:removeItem","markedbills", 20)   
		            cashPayment = cashPayment + markedbillsprice            
		            TriggerEvent("DoLongHudText","Thanks for the extra sauce!")
				elseif exports["ethical-inventory"]:hasEnoughOfItem("band",20,false) then     
    ...
    while timer > 0 do
        ...
        if not searching then
            ...
            if IsControlJustReleased(1,38) and #(pos - GetEntityCoords(PlayerPedId())) < 3.0 then
                ...
                local finished = exports["ethical-taskbar"]:taskBar(15000,"Searching Thug")
...
local drugLocs = {
	[1] =  { ['x'] = 450.67251586914, ['y'] = -1863.2703857422, ['z'] = 27.763427734375, ['h'] = 226.77166748047, ['info'] = ' Central', ["gang"] = "ballas", ["ent"] = 0 },
}
...
function CompleteScrapping(vehicle, originalVehicleLocation)
	...
	local finished = exports["ethical-taskbar"]:taskBar(20000,"Scrapping Car")
...
AddEventHandler('goldtrade', function()
	if exports["ethical-inventory"]:hasEnoughOfItem("goldbar",70,true) then
...
Citizen.CreateThread(function()
    ...
    while true do
      ...
      --  local dropOff3 = #(vector3(GetEntityCoords(PlayerPedId())) - vector3(chopinfo[3]["x"],chopinfo[3]["y"],chopinfo[3]["z"]))
        local dropOff4 = #(GetEntityCoords(PlayerPedId()) - vector3(-779.34063720703,-605.07690429688,4.072509765625))
        local dropOff5 = #(GetEntityCoords(PlayerPedId()) - vector3(-778.49670410156,-600.52746582031,30.274047851562))
		...
		if GoldBars < 1.5 then
			...
			if GoldBarTime then
				...
				if IsControlJustReleased(0,38) then
					if exports["ethical-inventory"]:hasEnoughOfItem("goldbar",70,true) then
						local finished = exports["ethical-taskbar"]:taskBar(120000,"Trading Bars")
						if finished == 100 then
        ...
        if dropOff4 < 1.5 then
            DrawText3Ds(-779.34063720703,-605.07690429688,4.072509765625, "[E] to Leave")
            if IsControlJustReleased(0,38) then
                CleanUpArea()
                SetEntityCoords(PlayerPedId(),-778.49670410156,-600.52746582031,30.274047851562)
            end
        end
		if dropOff5 < 1.5 then
            DrawText3Ds(-778.49670410156,-600.52746582031,30.274047851562, "[E] to Enter") 
            if IsControlJustReleased(0,38) then
                buildDrugShop()
                CreateDrugStorePed()
            end
	    	...
	    	if not isInVehicle then
	    		...
	    		if DoesEntityExist(veh) and vehResponse > 0 then
    				...
    				if IsControlJustReleased(0,38) then
						...
						TriggerEvent("chatMessagess", "EMAIL ", 8, message)
    				end
	    		end
	    	end
	    else
    ...
    while true do
		...
		elseif OxyRun then
			...
			if (not DoesEntityExist(oxyVehicle) or GetVehicleEngineHealth(oxyVehicle) < 100.0) and vehspawn then
				OxyRun = false
				tasking = false
				TriggerEvent("chatMessagess", "EMAIL - Drug Deliveries", 8, "Dude! You fucked the car up, I canceled your run, asshole! ")
			else
				if tasking then
			        Citizen.Wait(30000)
			    else
	...
	if watchinglist[gangpassed] then
		if watchinglist[gangpassed]["timer"] > 0 then
			...
			if myGangReputation[gangpassed] >= reputation then
				TriggerEvent("chatMessagess", "EMAIL ", 8, "Yo, just had someone hit me up in the " .. gangside .. " area")
			end
		end
	end
end)
