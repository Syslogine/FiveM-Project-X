ETHICAL.Admin.Menu = {}
ETHICAL._Admin.Menu = {}
ETHICAL._Admin.Menu.PlayerOptions = {}
ETHICAL._Admin.Menu.SearchOptions = {
    type = nil,
    data = nil
}
ETHICAL._Admin.Menu.Target = {}

ETHICAL._Admin.Menu.DevMode = false
ETHICAL._Admin.Menu.DevDebug = false

function ETHICAL.Admin.Menu.Init(self)
...
function ETHICAL.Admin.Menu.SetSubMenuProperties(self, menu)
    WarMenu.SetMenuWidth(menu, 0.5)
    WarMenu.SetTitleColor(menu, 135, 206, 250, 255)
    WarMenu.SetTitleBackgroundColor(menu, 0 , 0, 0, 150)
    WarMenu.SetMenuBackgroundColor(menu, 0, 0, 0, 100)
...
function ETHICAL.Admin.Menu.DrawCommand(self, cmd)
    if not cmd or not ETHICAL.Admin:CommandExists(cmd) then return end

    Citizen.CreateThread(function()
        while true do
            Citizen.Wait(0)
            if WarMenu.IsMenuOpened("command") then ETHICAL.Admin:GetCommandData(cmd).drawcommand() else return end
            if WarMenu.MenuButton("Back", "acommands") then return end
        end
    end)
end

function ETHICAL.Admin.Menu.DrawTargets(self, cmd, cb)
    WarMenu.OpenMenu("targetmenu")

    Citizen.CreateThread(function()
        while WarMenu.IsMenuOpened("targetmenu") do
            Citizen.Wait(0)
            for k,v in spairs(ETHICAL._Admin.Players, function(t, a, b) return t[a].source < t[b].source end) do
                if WarMenu.MenuButton("[" .. v.source .. "] " .. v.name, "command") then self:DrawCommand(cmd) cb(v) ETHICAL.Admin:GetCommandData(cmd).drawcommand() return end
            end
            if WarMenu.MenuButton("Back", "command") then self:DrawCommand(cmd) ETHICAL.Admin:GetCommandData(cmd).drawcommand() return end
        end
    end)
end

function ETHICAL.Admin.Menu.DrawJobs(self, cmd, cb)
    ...
    Citizen.CreateThread(function()
        while WarMenu.IsMenuOpened("jobmenu") do
            ...
            for k,v in spairs(jobs, function(t, a, b) return t[a].name < t[b].name end) do
                if WarMenu.MenuButton("[" .. v.name .. "] ", "command") then self:DrawCommand(cmd) cb(k) ETHICAL.Admin:GetCommandData(cmd).drawcommand() return end
            end
            if WarMenu.MenuButton("Back", "command") then self:DrawCommand(cmd) ETHICAL.Admin:GetCommandData(cmd).drawcommand() return end
        end
    end)
...
function ETHICAL.Admin.Menu.DrawTextInput(self, defaultText, cb)
    Citizen.CreateThread(function()
...
function ETHICAL.Admin.Menu.ShowTextEntry(self, title, subMsg, cb)
    SendNUIMessage({open = true, textEntry = true, title = title, submsg = subMsg and subMsg or ""})
    textEntryCb = function(text) cb(text) end
end

function ETHICAL.Admin.Menu.DrawRanks(self, cmd, cb)
    WarMenu.OpenMenu("ranklist")

    Citizen.CreateThread(function()
        while WarMenu.IsMenuOpened("ranklist") do
            Citizen.Wait(0)
            for k,v in spairs(ETHICAL.Admin:GetRanks(), function(t, a, b) return t[a].grant < t[b].grant end) do
                if WarMenu.MenuButton(k, "command") then self:DrawCommand(cmd) cb(k) ETHICAL.Admin:GetCommandData(cmd).drawcommand() return end
            end
            if WarMenu.MenuButton("Back", "command") then self:DrawCommand(cmd) ETHICAL.Admin:GetCommandData(cmd).drawcommand() return end
        end
    end)
...
Citizen.CreateThread(function()
    ...
    local function DrawPlayers()        
        for k,v in spairs(ETHICAL._Admin.Players, function(t, a, b) return t[a].source < t[b].source end) do
            if WarMenu.Button("[" .. v.source .. "] " .. v.name .. (v.sessioned and " - SESSIONED?" or ""), v.status, v.sessioned and {r = 255, g = 0, b = 0, a = 255} or nil) then ETHICAL._Admin.Menu.PlayerOptions = v viewingIp = false viewingLicense = false WarMenu.OpenMenu("aplayeropts") end
        end
    ...
    local function DrawDiscPlayers()
        for k,v in spairs(ETHICAL._Admin.DiscPlayers, function(t, a, b) return t[a].source < t[b].source end) do
            if WarMenu.MenuButton("[" .. v.source .. "] " .. v.name, "aplayeropts") then ETHICAL._Admin.Menu.PlayerOptions = v viewingIp = false end
        end
        if WarMenu.MenuButton("Back", "aplayers") then return end
    end

    local function DrawPlayerOptions()
        local options = ETHICAL._Admin.Menu.PlayerOptions
    ...
    local function DrawCommands()
        local userRank = ETHICAL.Admin:GetPlayerRank()

        if cat then
            for k,v in spairs(ETHICAL.Admin:GetCommands(), function(t, a, b) return t[a].title < t[b].title end) do
                if v.category == cat then
                    WarMenu.SetLastCat(cat)
                    if ETHICAL.Admin:RankHasCommand(userRank, v.command) then if WarMenu.Button(v.title) then WarMenu.OpenMenu("command") ETHICAL.Admin.Menu:DrawCommand(v.command) end end
                end
            end
        end
    ...
    local function DrawCategories()
        for k,v in spairs(ETHICAL.Admin:GetCategories(), function(t, a, b) return a < b end) do
    ...
    local function DrawDev()
        local userRank = ETHICAL.Admin:GetPlayerRank()
        if WarMenu.Button("Dev mode: " .. (ETHICAL._Admin.Menu.DevMode and "Disable" or "Enable")) and userRank == "dev" then 
            ETHICAL._Admin.Menu.DevMode = not ETHICAL._Admin.Menu.DevMode 
            TriggerEvent("ethical-admin:currentDevmode",ETHICAL._Admin.Menu.DevMode)
        end
        if WarMenu.Button("Dev debug: " .. (ETHICAL._Admin.Menu.DevDebug and "Disable" or "Enable")) and userRank == "dev" then 
            ETHICAL._Admin.Menu.DevDebug = not ETHICAL._Admin.Menu.DevDebug 
            TriggerEvent("ethical-admin:currentDebug",ETHICAL._Admin.Menu.DevDebug)
            TriggerServerEvent("server:enablehuddebug", nil,ETHICAL._Admin.Menu.DevDebug)
        end
        if WarMenu.MenuButton("Back", "amenu") then return end
    end
    ...
    --     local userRank = ETHICAL.Admin:GetPlayerRank()
    --     if WarMenu.MenuButton("Ban Search", "searchopts") then ETHICAL._Admin.Menu.SearchOptions.type = "ban" end
    --     if WarMenu.MenuButton("Character Search", "searchopts") then ETHICAL._Admin.Menu.SearchOptions.type = "character" end
    --     if WarMenu.MenuButton("Steam ID Search", "searchopts") then ETHICAL._Admin.Menu.SearchOptions.type = "steamid" end
    --     if WarMenu.Button("Dump Current Players") then ETHICAL.Admin.DumpCurrentPlayers() end
    -- end
    ...
    --     local type = ETHICAL._Admin.Menu.SearchOptions.type
    ...
    --         ETHICAL.Admin.Menu:ShowTextEntry(title, "", function(result)
    --             if result then
    --                 searching = true
    ...
    --             TriggerServerEvent("ethical-admin:searchRequest", ETHICAL._Admin.SearchOptions.type, result)
    --         end)
    --     end
...
RegisterNetEvent("ethical-admin:openMenu")
AddEventHandler("ethical-admin:openMenu", function()
    WarMenu.OpenMenu("amenu")
end)

RegisterNetEvent("ethical-admin:drawLastCat")
AddEventHandler("ethical-admin:drawLastCat", function(sentCat)
    cat = sentCat
    WarMenu.OpenMenu("acommands")
end)

RegisterNetEvent("ethical-admin:currentDebug")
AddEventHandler("ethical-admin:currentDebug", function(debugToggle)
    ETHICAL._Admin.Menu.DevDebug = debugToggle
end)
