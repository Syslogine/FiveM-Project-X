...
ETHICAL._Admin.Ranks = {}
ETHICAL._Admin.Commands = {}
ETHICAL._Admin.Players = {}
ETHICAL._Admin.DiscPlayers = {}
ETHICAL._Admin.CurAdmins = {}

function ETHICAL.Admin.Log(self, log, user)
...
function ETHICAL.Admin.ChatPrint(self, msg, target)

end

function ETHICAL.Admin.RankExists(self, rank)
    rank = tostring(rank)
...
function ETHICAL.Admin.CommandExists(self, cmd)
    cmd = tostring(cmd)
...
function ETHICAL.Admin.RankInheritsCommand(self, rank, cmd)
...
function ETHICAL.Admin.GetPlayerRank(self, user)
    if not IsDuplicityVersion() then
        return exports["ethical-base"]:getModule("LocalPlayer"):getVar("rank")
    else
...
function ETHICAL.Admin.RankHasCommand(self, rank, cmd)
...
function ETHICAL.Admin.GetRanks(self)
    return ETHICAL._Admin.Ranks
end

function ETHICAL.Admin.GetRankData(self, rank)
    if not rank then return false end
...
function ETHICAL.Admin.GetCommands(self)
    return ETHICAL._Admin.Commands
end

function ETHICAL.Admin.GetCategories(self)
    local tmp = {}
...
function ETHICAL.Admin.GetCommandData(self, cmd)
    if not cmd then return false end
...
function ETHICAL.Admin.IsAdmin(self, user)
    if not user then return false end
...
function ETHICAL.Admin.IsSuperAdmin(self, user)
    if not IsDuplicityVersion() then
...
function ETHICAL.Admin.IsValidUser(self, user)
    if not user then return false end
...
function ETHICAL.Admin.CanTarget(self, user, target)
...
function ETHICAL.Admin.CanTargetRank(self, user, rank)
...
function ETHICAL.Admin.AddCommand(self, cmd)
    local tmp = {}
    ...
    if IsDuplicityVersion() then
        ...
        tmp.runcommand = function(args) args.command = tmp.command ETHICAL.Admin:RunCommand(args) end
...
function ETHICAL.Admin.AddRank(self, rank, data)
    local tmp = {}
...
function ETHICAL.Admin.GetBanTimeFromString(self, time)
...
function ETHICAL.Admin.SetStatus(self, status, src)
    ...
    if IsDuplicityVersion() then
        ...
        if not player then else ETHICAL._Admin.Players[src].status = status end

        for k,v in pairs(ETHICAL._Admin.CurAdmins) do
            TriggerClientEvent("ethical-admin:setStatus", k, src, status)
        end

        return
    end

    TriggerServerEvent("ethical-admin:setStatus", status)
end

function ETHICAL.Admin.DumpCurrentPlayers(self)
    TriggerServerEvent("admin:dumpCurrentPlayers",ETHICAL._Admin.Players,ETHICAL._Admin.DiscPlayers)
end

AddEventHandler("ethical-base:exportsReady", function()
    exports["ethical-base"]:addModule("Admin", ETHICAL.Admin)
    exports["ethical-base"]:addModule("_Admin", ETHICAL._Admin)
end)