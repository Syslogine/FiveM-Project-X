...
function cmd.DrawCommand()
        ...
        if WarMenu.Button("Select a target", "Selected: " .. (cmd.vars.target and cmd.vars.target.name or "None")) then ETHICAL.Admin.Menu:DrawTargets(cmd.command, function(_target)
            cmd.vars.target = _target
        end) end           

        if WarMenu.Button("Select a rank", "Selected: " .. (cmd.vars.rank ~= nil and tostring(cmd.vars.rank) or "None")) then ETHICAL.Admin.Menu:DrawRanks(cmd.command, function(_rank)
            cmd.vars.rank = _rank
        end) end
        ...
        if args.target and args.rank then if WarMenu.Button("Set " .. args.target.name .. "'s rank to " .. args.rank) then ETHICAL.Admin:GetCommandData(cmd.command).runcommand(args) cmd.vars.target = nil cmd.vars.rank = nil end end
end

ETHICAL.Admin:AddCommand(cmd)
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("God Mode: " .. (cmd.vars.toggle and "Disable" or "Enable")) then cmd.vars.toggle = not cmd.vars.toggle ETHICAL.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle}) end
end
...
function cmd.RunClCommand(args)
    ETHICAL.Admin.teleportMarker()
end

function cmd.DrawCommand()      
    if WarMenu.Button("Teleport to marker") then ETHICAL.Admin:GetCommandData(cmd.command).runcommand({}) end
end
...
function cmd.DrawCommand()      
    ...
    if WarMenu.Button("Enter x", "x: " .. (cmd.vars.x and cmd.vars.x or "0.0")) then
        ETHICAL.Admin.Menu:ShowTextEntry("Enter Item", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end

            if string.find(result, ",") then
                local resultSplit = ETHICAL.Admin.split(result, ',')
    ...
    if WarMenu.Button("Enter y", "y: " .. (cmd.vars.y and cmd.vars.y or "0.0")) then
        ETHICAL.Admin.Menu:ShowTextEntry("Enter y", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if WarMenu.Button("Enter z", "z: " .. (cmd.vars.z and cmd.vars.z or "0.0")) then
        ETHICAL.Admin.Menu:ShowTextEntry("Enter z", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if cmd.vars.x and cmd.vars.y and cmd.vars.z then if WarMenu.Button("Teleport to: " .. "X:["..cmd.vars.x.."] Y:["..cmd.vars.y.."] Z:["..cmd.vars.z.."]") then ETHICAL.Admin:GetCommandData(cmd.command).runcommand({x = cmd.vars.x,y = cmd.vars.y,z = cmd.vars.z}) end end
end
ETHICAL.Admin:AddCommand(cmd)
...
function cmd.Init()
    ...
    RegisterNetEvent("ethical-admin:NoclipState")
    AddEventHandler("ethical-admin:NoclipState", function(toggle)
...
function cmd.RunClCommand(args)
    cmd.vars.enable = args.toggle
    ETHICAL.Admin:RunNclp(cmd.vars.enable)
end

function cmd.DrawCommand()
    cmd.vars.toggle = cmd.vars.toggle ~= nil and cmd.vars.toggle or nil
    if WarMenu.Button("Noclip: " .. (cmd.vars.toggle and "Disable" or "Enable")) then cmd.vars.toggle = not cmd.vars.toggle ETHICAL.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle}) end
end
...
function cmd.DrawCommand()
    ...
    if cmd.vars.target then if WarMenu.Button("Teleport to " .. cmd.vars.target.name .. "'s position") then ETHICAL.Admin:GetCommandData(cmd.command).runcommand({target = cmd.vars.target, retn = false}) end end
    if cmd.vars.lastPos then if WarMenu.Button("Return to your last position") then ETHICAL.Admin:GetCommandData(cmd.command).runcommand({target = false, retn = true}) end end
end
...
function cmd.DrawCommand()
    ...
    if cmd.vars.target then if WarMenu.Button("Bring " .. cmd.vars.target.name .. " to you.") then ETHICAL.Admin:GetCommandData(cmd.command).runcommand({target = cmd.vars.target, retn = false}) end end
end
...
function cmd.DrawCommand()
    ...
    cmd.vars.toggle = cmd.vars.toggle ~= nil and cmd.vars.toggle or nil
    if WarMenu.Button("Attach: " .. (cmd.vars.toggle and "Disable" or "Enable")) then cmd.vars.toggle = not cmd.vars.toggle ETHICAL.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle,target = cmd.vars.target}) end
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter a reason", "Reason: " .. (cmd.vars.reason and cmd.vars.reason or "No Reason Given")) then
        ETHICAL.Admin.Menu:DrawTextInput(cmd.vars.reason and cmd.vars.reason or "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if cmd.vars.target then if WarMenu.Button("Kick " .. cmd.vars.target.name) then ETHICAL.Admin:GetCommandData(cmd.command).runcommand({target = cmd.vars.target, reason = cmd.vars.reason}) end end
end
...
function cmd.RunCommand(caller, args)
    ...
    TriggerEvent("ethical-admin:update:vehicle", caller.source, args.reason)
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Set State") then cmd.vars.toggle = not cmd.vars.toggle ETHICAL.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle, reason = cmd.vars.reason}) end
end
...
function cmd.DrawCommand()
    ...
    cmd.vars.toggle = cmd.vars.toggle ~= nil and cmd.vars.toggle or nil
    if WarMenu.Button("Search Player") then cmd.vars.toggle = not cmd.vars.toggle ETHICAL.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle,target = cmd.vars.target}) end
end
...
function cmd.RunCommand(caller, args)
    ...
    TriggerClientEvent("ethical-admin:Cloak", -1, src, args.toggle)
...
function cmd.Init()
    ...
    if IsDuplicityVersion() then 
        AddEventHandler("ethical-base:characterLoaded", function(user, char)
            ...
            TriggerClientEvent("ethical-admin:Cloak", src, cmd.vars.cloaked)
        end)

        AddEventHandler("playerDropped", function()
            local src = source
            if cmd.vars.cloaked[src] then
                TriggerClientEvent("ethical-admin:Cloak", -1, src, false)
    ...
    RegisterNetEvent("ethical-admin:CloakRemote")
    AddEventHandler("ethical-admin:CloakRemote", function()
        if ETHICAL.Admin:GetPlayerRank() == "dev" then
            cmd.vars.toggle = not cmd.vars.toggle
            ETHICAL.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle})
        end
    end)

    RegisterNetEvent("ethical-admin:Cloak")
    AddEventHandler("ethical-admin:Cloak", function(player, toggle)
...
function cmd.DrawCommand()
    cmd.vars.toggle = cmd.vars.toggle ~= nil and cmd.vars.toggle or nil
    if WarMenu.Button("Cloak: " .. (cmd.vars.toggle and "Disable" or "Enable")) then cmd.vars.toggle = not cmd.vars.toggle ETHICAL.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle}) end
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter a message", "Message: " .. (cmd.vars.message and cmd.vars.message or "No Message")) then
        ETHICAL.Admin.Menu:ShowTextEntry("Enter a Message", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if cmd.vars.message then if WarMenu.Button("Send Message: " .. cmd.vars.message) then ETHICAL.Admin:GetCommandData(cmd.command).runcommand({message = cmd.vars.message}) end end
end
...
function cmd.RunCommand(caller, args)
    if not args.message then return end
    local src = caller:getVar("source")
    TriggerClientEvent("ethical-admin:runSpawnCommand",src,args.message)
    exports["ethical-log"]:AddLog("Admin", caller, "Spawned car", {car = tostring(args.message)}) 
    ETHICAL.Admin:Log(log, caller)
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter a model", "Model: " .. (cmd.vars.message and cmd.vars.message or "No model")) then
        ETHICAL.Admin.Menu:ShowTextEntry("Enter a model", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if WarMenu.Button("Seat into Vehicle") then TriggerEvent("ethical-admin:SeatIntoLast") end

    if cmd.vars.message then if WarMenu.Button("Spawn model: " .. cmd.vars.message) then ETHICAL.Admin:GetCommandData(cmd.command).runcommand({message = cmd.vars.message}) end end
end
...
--     ETHICAL.Admin.DB:giveCar(args.message,args.reciver)
...
--         ETHICAL.Admin.Menu:ShowTextEntry("Enter a cid", "", function(result)
--             if result then
--                 if string.gsub(result, " ", "") == "" or result == "" then result = nil end
--             end
...
--     if cmd.vars.message and cmd.vars.reciver then if WarMenu.Button("Give model: " .. cmd.vars.message.." to "..cmd.vars.reciver) then ETHICAL.Admin:GetCommandData(cmd.command).runcommand({message = cmd.vars.message,reciver = cmd.vars.reciver}) end end
-- end
...
function cmd.RunCommand(caller, args)
    ...
    exports["ethical-log"]:AddLog("Admin", caller, "Spawned item", {item = tostring(args.message),amount = tostring(args.amount)}) 
    ETHICAL.Admin:Log(log, caller)
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter Amount", "Amount: " .. (cmd.vars.amount and cmd.vars.amount or "1")) then
        ETHICAL.Admin.Menu:ShowTextEntry("Enter Amount", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if cmd.vars.message then if WarMenu.Button("Spawn Item: " .. cmd.vars.message.." | "..cmd.vars.amount) then ETHICAL.Admin:GetCommandData(cmd.command).runcommand({message = cmd.vars.message,amount = cmd.vars.amount}) end end
end
...
function cmd.RunCommand(caller, args)
    ...
    exports["ethical-log"]:AddLog("Admin", caller, "Changed weather/time/Cycle", {time = args.time, weather = args.weather, light = args.light, blackout = args.blackout}) 
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter a time", "Time: " .. (cmd.vars.time and cmd.vars.time or "0")) then
        ETHICAL.Admin.Menu:ShowTextEntry("Enter Time", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if WarMenu.Button("Enter a Weather status", "Weather: " .. (cmd.vars.weather and cmd.vars.weather or "none")) then
        ETHICAL.Admin.Menu:ShowTextEntry("Enter Weather status", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if WarMenu.Button("Enter a Cycle Effect", "Cycle: " .. (cmd.vars.lightType and cmd.vars.lightType or "none")) then
        ETHICAL.Admin.Menu:ShowTextEntry("Enter Time Cycle Effect", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if cmd.vars.time or cmd.vars.weather or cmd.vars.lightType then if WarMenu.Button("Update Server/Client") then ETHICAL.Admin:GetCommandData(cmd.command).runcommand({time = cmd.vars.time, weather = cmd.vars.weather, light = cmd.vars.lightType, blackout = cmd.vars.toggle}) end end
end
...
function cmd.RunCommand(caller, args)
    ...
    exports["ethical-log"]:AddLog("Admin", caller, "Set Model", {item = tostring(args.message)}) 
    ETHICAL.Admin:Log(log, caller)
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter model name", "Model: " .. (cmd.vars.message and cmd.vars.message or "No model")) then
        ETHICAL.Admin.Menu:ShowTextEntry("Enter Model", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if cmd.vars.message then if WarMenu.Button("Set Model: " .. cmd.vars.message) then ETHICAL.Admin:GetCommandData(cmd.command).runcommand({message = cmd.vars.message}) end end
end
...
function cmd.DrawCommand()
    ...
    if cmd.vars.message then if WarMenu.Button("Send Announcement: " .. cmd.vars.message) then ETHICAL.Admin:GetCommandData(cmd.command).runcommand({message = cmd.vars.message}) end end
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Revive in Distance") then 
        TriggerEvent("ethical-admin:ReviveInDistance")
    end


    if cmd.vars.target then if WarMenu.Button("Revive and Heal " .. cmd.vars.target.name) then ETHICAL.Admin:GetCommandData(cmd.command).runcommand({target = cmd.vars.target, runontarget = true}) end end
end
...
function cmd.DrawCommand()
    ...
    if cmd.vars.target then if WarMenu.Button("Fix " .. cmd.vars.target.name .. "'s vehicle") then ETHICAL.Admin:GetCommandData(cmd.command).runcommand({target = cmd.vars.target, runontarget = true}) end end
    if GetVehiclePedIsIn(PlayerPedId(), false) ~= 0 then if WarMenu.Button("Fix Current vehicle.") then ETHICAL.Admin:GetCommandData(cmd.command).runcommand({runontarget = false}) end end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Select a job", "Selected: " .. (cmd.vars.target and cmd.vars.target or "None")) then ETHICAL.Admin.Menu:DrawJobs(cmd.command, function(_target)
        cmd.vars.target = _target
    end) end

    if cmd.vars.target then if WarMenu.Button("Request [" .. cmd.vars.target .. "]") then ETHICAL.Admin:GetCommandData(cmd.command).runcommand({target = cmd.vars.target}) end end
end
...
function cmd.Init()
    ...
    ETHICAL.Admin.Menu:SetSubMenuProperties("filtermenu")
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Entity View: " .. (cmd.vars.toggle and "Disable" or "Enable")) then cmd.vars.toggle = not cmd.vars.toggle ETHICAL.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle, filter = cmd.vars.filter}) end

    if WarMenu.Button("Enter view distance", "Entered: " .. (cmd.vars.distance or "150")) then 
        ETHICAL.Admin.Menu:ShowTextEntry("Enter a distance", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
        ...
        Citizen.CreateThread(function()
            while WarMenu.IsMenuOpened("targetmenu") do
                ...
                for f, t in pairs(cmd.vars.filters) do
                    if WarMenu.MenuButton(f, "command") then cmd.vars.filter = f ETHICAL.Admin.Menu:DrawCommand(cmd.command) return end
                end
                if WarMenu.MenuButton("Back", "command") then ETHICAL.Admin.Menu:DrawCommand(cmd.command) return end
            end
        end)
    end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter an entity ID") then 
        ETHICAL.Admin.Menu:ShowTextEntry("Enter an entity ID", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
                result = tonumber(result)
                if not result then result = nil end
    ...
    if cmd.vars.ent then if WarMenu.Button("Delete Entity", "Entity: " .. (cmd.vars.ent or "none")) then
        ETHICAL.Admin:GetCommandData(cmd.command).runcommand({ent = cmd.vars.ent})
    end end
end
...
--[[
    function cmd.ChatCommand(args)
        ...
        if not ETHICAL.Admin:RankExists(rank) then return end
        if not ETHICAL.Admin:IsValidUser(target) then return end

        local args = {
            target = exports["ethical-base"]:getModule("Player"):getUser(target),
            rank = rank
        }
