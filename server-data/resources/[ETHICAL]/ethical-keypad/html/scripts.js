...
document.onkeyup = function (data) {
  if (data.which == 27 ) {
    $.post('http://ethical-keypad/close', JSON.stringify({}));
  } else if (data.which == 13 ) {
    $.post('http://ethical-keypad/complete', JSON.stringify({ pin: $( "#PINbox" ).val() }));
  } else {
    if ( !isNaN(data.key) ) {
      var v = $( "#PINbox" ).val();
...
function submitForm(e) {
  if (e.value == "") {
     ...
     $.post('http://ethical-keypad/complete', JSON.stringify({ pin:e.value }));
    data = {
      pin: e.value
    }
    /*    
