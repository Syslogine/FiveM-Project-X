...
function attemptToRob(itemID,activePolice)
    ...
    if itemID == "lockpick" then
        ...
        local finished = exports["ethical-lockpicking"]:lockpick(pick[1],pick[2],pick[3],pick[4])
    ...
    if itemID == "thermite" then
            ...
            local outcome = exports["ethical-thermite"]:startGame(thermite[1],thermite[2],thermite[3],thermite[4])
            local finished = 0
            ...
            if finished == 100 then
                ...
                TriggerServerEvent('ethical-robbery:checkflag')
                
            else
                if math.random(1,100) > 85 then
                    exports["ethical-thermite"]:startFireAtLocation(marker.pos[1],marker.pos[2],marker.pos[3]-0.3,10000)                   
                end
    ...
    if itemID == "Gruppe6Card2" then
        ...
        if exports["ethical-inventory"]:hasEnoughOfItem("electronickit",1,false) then
            local card = exports["ethical-taskbar"]:taskBar(9000,"Inserting Card")
            if card == 100 then
                Wait(400)
                TriggerEvent("animation:cancel")
                TriggerServerEvent("robbery:alarmTrigger",locationID)
                local rdif = exports["ethical-taskbar"]:taskBar(6000,"Hacking..")
                if  rdif == 100 then
                    local kit = difficult.kit
...
function thermiteHandle(locationID,itemID)
    ...
    local taskFinished = exports["ethical-taskbar"]:taskBar(1000,"Getting Thermite ready")
