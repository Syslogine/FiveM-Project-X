   ...
   -- print("door id: "..marker.attachedDoor)
    if marker.attachedDoor ~= nil and marker.attachedDoor ~= 0 and marker.attachedDoor ~= -22 then 
       -- print("i am about to unlock the door rn bud")
        ...
        TriggerEvent('ethical-doors:ForceLockState', marker.attachedDoor, 0) end
    ...
    for k,v in pairs(markers) do
        if v.group == GroupType and v.ToolType == ELECTRICAL_LOCK then
            ...
            if v.attachedDoor ~= nil and v.attachedDoor ~= -22 and v.attachedDoor ~= 0 then
                TriggerClientEvent("ethical-doors:ForceLockState",v.attachedDoor,0)
            end
        end
    end
end
...
function lockdown()
    ...
    TriggerEvent("ethical-doors:ForceLockState",211,1)
    TriggerEvent("ethical-doors:ForceLockState",212,1)
    TriggerEvent("ethical-doors:ForceLockState",213,1)
    TriggerEvent("ethical-doors:ForceLockState",214,1)
end

function endLockdown()
    TriggerClientEvent("jail:lockdown", -1, false)
    TriggerEvent("ethical-doors:ForceLockState",211,0)
    TriggerEvent("ethical-doors:ForceLockState",212,0)
    TriggerEvent("ethical-doors:ForceLockState",213,0)
    TriggerEvent("ethical-doors:ForceLockState",214,0)
end

function CheckLargeBanks()
    for k,v in pairs(markers) do
        if v.attachedDoor ~= nil and v.attachedDoor ~= -22 and v.attachedDoor ~= 0 then
            if not flags[k].isFinished and not exports["ethical-doors"]:isDoorLocked(v.attachedDoor) then
                TriggerEvent("ethical-doors:ForceLockState", v.attachedDoor,1)
            end
        end
...
-- RegisterServerEvent('ethical-robbery:checkflag')
-- AddEventHandler('ethical-robbery:checkflag', function(data)
--     -- Prison_Power
--     local check = false
...
RegisterServerEvent('ethical-robbery:checkflag')
AddEventHandler('ethical-robbery:checkflag', function()
    -- Prison_Power
   total = total + 1
  -- print(total)
