...
AddEventHandler('Stealtheybread', function(target)
    ...
    local targetply = exports["ethical-base"]:getModule("Player"):GetUser(target)
    user:addMoney(targetply:getCash())
    targetply:removeMoney(targetply:getCash())
end)


RegisterNetEvent('ethical-weapons:getAmmo')
AddEventHandler('ethical-weapons:getAmmo', function()
    local ammoTable = {}
    local src = source
	local user = exports["ethical-base"]:getModule("Player"):GetUser(src)
    ...
    exports.ghmattimysql:execute("SELECT type, ammo FROM characters_weapons WHERE id = @id", {['id'] = char.id}, function(result)
        ...
        TriggerClientEvent('ethical-items:SetAmmo', src, ammoTable)
    end)
end)

RegisterNetEvent('ethical-weapons:updateAmmo')
AddEventHandler('ethical-weapons:updateAmmo', function(newammo,ammoType,ammoTable)
    local src = source
	local user = exports["ethical-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
...
RegisterServerEvent("ethical-inventory:update:settings")
AddEventHandler("ethical-inventory:update:settings", function(data)
    local src = source
