...
onNet("inventory-degItem", async (itemID) => {
    let amount = 4000000
    exports.oxmysql.execute(`UPDATE user_inventory2 set creationDate = ${amount} WHERE id = ${itemID}`, {}, function() {});
});
...
onNet("server-inventory-open", async (coords, player, secondInventory, targetName, itemToDropArray, sauce) => {
    ...
    //    emitNet('current-items', src, invArray)
        ...
        if (secondInventory == "1") {
            ...
            var shopAmount = 1;
            emitNet("inventory-open-target", src, [invArray, arrayCount, playerinvname, shopArray, shopAmount, targetinvname, 500, false]);
        } else if (secondInventory == "1234") {
            var targetinvname = targetName;
            var shopArray = burgiestore();
            var shopAmount = 8;
            emitNet("inventory-open-target", src, [invArray, arrayCount, playerinvname, shopArray, shopAmount, targetinvname, 500, false]);
        } else if (secondInventory == "8967") {
            var targetinvname = targetName;
            var shopArray = ethicalburgershot();
            var shopAmount = 5;
            emitNet("inventory-open-target", src, [invArray, arrayCount, playerinvname, shopArray, shopAmount, targetinvname, 500, false]);
        } else if (secondInventory == "950") {
            var targetinvname = targetName;
            var shopArray = burgerdrinks();
            var shopAmount = 2;
            emitNet("inventory-open-target", src, [invArray, arrayCount, playerinvname, shopArray, shopAmount, targetinvname, 500, false]);
        } else if (secondInventory == "6564") {
            var targetinvname = targetName;
            var shopArray = ethicalburgershotorder();
            var shopAmount = 7;
            emitNet("inventory-open-target", src, [invArray, arrayCount, playerinvname, shopArray, shopAmount, targetinvname, 500, false]);
...
onNet("inv:playerSpawned", () => {
    ...
    console.log("[ethical-inventory] Drops & broken items were deleted.")
});
