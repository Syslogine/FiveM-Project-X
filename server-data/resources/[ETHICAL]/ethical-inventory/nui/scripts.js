...
$(document).ready(function () {
    $('.save-settings').click(() => {
        ...
        $.post(
            'https://ethical-inventory/UpdateSettings',
            JSON.stringify({
    ...
    window.addEventListener('message', function (event) {
        ...
        if (item.response == 'openGui') {
            ...
            if (itemList[item.id]) {
                $.post(
                    'https://ethical-inventory/GiveItem',
                    JSON.stringify([
                        item.id,
                        item.amount,
...
function invStack(
    ...
    $.post('https://ethical-inventory/stack', JSON.stringify(arr));
}
...
function invMove(
    ...
    $.post('https://ethical-inventory/move', JSON.stringify(arr));
}
...
function invSwap(targetSlot, targetInventory, originSlot, originInventory, itemid1, metainformation1, itemid2, metainformation2) {
    ...
    $.post('https://ethical-inventory/swap', JSON.stringify(arr));
}

function removeCraftItems(itemid, moveAmount) {
    let arr = itemList[itemid].craft;
    let amount = moveAmount;
    $.post('https://ethical-inventory/removeCraftItems', JSON.stringify([arr, amount]));
}
...
function DisplayInventoryMultiple(playerinventory, itemCount, invName, targetinventory, targetitemCount, targetinvName, cash, Owner, targetInvWeight = 0) {
    ...
    } else if (targetinvName.indexOf('ethical-jobmanager_shelf') > -1) {
        secondaryMaxWeight = 250.0;
        slotLimitTarget = 40;
        displayName = 'Burger Shelf'
    } else if (targetinvName.indexOf('ethicalburgershot_pickup') > -1) {
        secondaryMaxWeight = 100.0;
        slotLimitTarget = 10;
        displayName = 'Pickup Order';
    } else if (targetinvName.indexOf('ethicalburgershot_pickup2') > -1) {
        secondaryMaxWeight = 100.0;
        slotLimitTarget = 10;
        displayName = 'Pickup Order';
    } else if (targetinvName.indexOf('ethicalburgershot_pickup3') > -1) {
        secondaryMaxWeight = 100.0;
        slotLimitTarget = 10;
        displayName = 'Pickup Order';
...
function BuildDrop(brokenSlots) {
    $.post(
        'https://ethical-inventory/dropIncorrectItems',
        JSON.stringify({
            slots: brokenSlots,
        }),
    );
...
function UpdateSetWeights(secondaryName) {
    ...
    $.post(
        'https://ethical-inventory/Weight',
        JSON.stringify({
...
function DragToggle(fromSlot, isUsing, mouseEvent) {
    ...
    if (fromSlot) {
        ...
        if (occupiedslot && !isDragging) {
            ...
            $.post('https://ethical-inventory/SlotInuse', JSON.stringify(parseInt(draggingid.replace(/\D/g, ''))));
...
function DropItem(slot, amountDropped) {
    ...
    // $.post('https://ethical-inventory/dropitem', JSON.stringify({
...
function ErrorMove() {
    // $.post('https://ethical-inventory/move:fail', JSON.stringify({}));
}

function SuccessMove() {
    // $.post('https://ethical-inventory/move:success', JSON.stringify({}));
}
...
function CompileStacks(
    ...
    $.post(
        'https://ethical-inventory/SlotJustUsed',
        JSON.stringify({
...
function SwapStacks(targetSlot, originSlot, inv1, inv2, itemid1, metainformation1, itemid2, metainformation2) {
    // $.post('https://ethical-inventory/swapstack', JSON.stringify({
    //   slot1: slot1,
    //   slot2: slot2,
    //   inv1: inv1,
    //   inv2: inv2,
...
function closeInv(pIsItemUsed = false) {
    ...
    $.post(
        'https://ethical-inventory/ServerCloseInventory',
        JSON.stringify({
...
function useitem() {
    ...
    if (inventoryUsedName == PlayerInventoryName) {
        ...
        $.post('https://ethical-inventory/invuse', JSON.stringify(arr));
