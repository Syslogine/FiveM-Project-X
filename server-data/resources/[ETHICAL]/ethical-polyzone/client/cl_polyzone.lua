...
local function addToComboZone(zone)
    if comboZone ~= nil then
        ...
        comboZone = ComboZone:Create({ zone }, { name = "ethical-polyzone" })
        comboZone:onPlayerInOutExhaustive(function(isPointInside, point, insideZones, enteredZones, leftZones)
            if leftZones ~= nil then
              for i = 1, #leftZones do
                TriggerEvent("ethical-polyzone:exit", leftZones[i].name)
              end
            end
            if enteredZones ~= nil then
              for i = 1, #enteredZones do
                TriggerEvent("ethical-polyzone:enter", enteredZones[i].name, enteredZones[i].data, enteredZones[i].center)
              end
            end
        end, 500)
    end
...
RegisterNetEvent("ethical-polyzone:createCircleZone")
AddEventHandler("ethical-polyzone:createCircleZone", function(name, ...)
  addCircleZone(name, ...)
end)
