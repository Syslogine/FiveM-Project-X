...
$(document).ready(function(){
  ...
  function AttemptComplition() {
    ...
    if(isFailed)
    {
      $.post('http://ethical-cid/error', JSON.stringify({  message: failureMessage}));
    }
    else
    {
      $.post('http://ethical-cid/create', JSON.stringify({first: first, last: Last,job: Job,sex: Sex,dob: DOB }));
    }
  ...
  window.addEventListener('message', function(event){
    ...
    if(item.openPhone === false) {
     ...
     document.onkeyup = function (data) {
      if (data.which == 27 ) {
        $.post('http://ethical-cid/close', JSON.stringify({}));
      }
    }
  });
