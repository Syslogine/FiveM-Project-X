...
function Login.CreatePlayerCharacterPeds(characterModelData,isReset)
    ...
    local events = exports["ethical-base"]:getModule("Events")
    events:Trigger("ethical-base:fetchPlayerCharacters", nil, function(data)
        if data.err then
            return
        end
...
--[[
    ...
    Description: dealing with ethical-base functionality in order to set/get the correct information for chars
]]
...
function Login.getCharacters(isReset)
    ...
    if not isReset then
        ...
        events:Trigger("ethical-base:loginPlayer", nil, function(data)
...
function Login.SelectedChar(data)
	...
	local events = exports["ethical-base"]:getModule("Events")
	events:Trigger("ethical-base:selectCharacter", data.actionData, function(returnData)
        ...
        if Login.CurrentClothing[data.actionData] == nil then
	        ...
	        TriggerEvent("ethical-base:firstSpawn")
            ...
            TriggerEvent('ethical-inventory:syncinventory')
...
RegisterNetEvent("ethical-spawn:finishedClothing")
AddEventHandler("ethical-spawn:finishedClothing", function(endType)
    ...
    if distance <= 10 then
    	...
    	if endType == "Finished" then
            ...
            Citizen.Wait(2000)
            if not exports["ethical-inventory"]:hasEnoughOfItem("idcard",1,false) then
                TriggerEvent("player:receiveItem","idcard",1,true)
            end
            if not exports["ethical-inventory"]:hasEnoughOfItem("mobilephone",1,false)then
                TriggerEvent("player:receiveItem","mobilephone",1)
            end
            if not exports["ethical-inventory"]:hasEnoughOfItem("present",1,false)then
                TriggerEvent("player:receiveItem","present",1)
            end
            TriggerServerEvent('ethical-base:addLicenses')
            Citizen.Wait(2000)
