...
function Login.NUICallback(data)
	...
	if data.action == "currentHover" then
		...
		local coords = exports["ethical-base"]:getModule("Util"):ScreenRelToWorld(GetCamCoord(LoginSafe.Cam), GetCamRot(LoginSafe.Cam,2),offset)
	...
	if data.action == "deleteCharacter" then
		...
		events:Trigger("ethical-base:deleteCharacter", data.actionData, function(deleted)
            Login.getCharacters(true)
        end)
	end
	...
	if data.action == "newCharacter" then
		...
		events:Trigger("ethical-base:createCharacter", chardata, function(created)
            if not created then
...
RegisterNetEvent("ethical-base:spawnInitialized")
...
RegisterNetEvent("spawn:logout")
AddEventHandler("spawn:logout", function(AlreadySpawned)
	isNear = true
	ClearFocus()
	DestroyAllCams(true)
	RenderScriptCams(false, true, 1, true, true)
