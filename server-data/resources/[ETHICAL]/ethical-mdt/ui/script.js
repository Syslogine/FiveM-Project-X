    ...
    methods: {
        changePage(page) {
            ...
            if (page == "Home") {
                ...
                $.post('http://ethical-mdt/getWarrants', JSON.stringify({}));
        ...
        closeMDT() {
            $.post('http://ethical-mdt/close', JSON.stringify({}));
        },
        OffenderSearch() {
            if (this.offender_search) {

                this.offender_results.query = this.offender_search;
                $.post('http://ethical-mdt/performOffenderSearch', JSON.stringify({
        ...
        OpenOffenderDetails(id) {
            // console.log("OPEN OFFENDERS ID", id)
            for (var key in this.offender_results.results) {
                if (id == this.offender_results.results[key].id) {
                    console.log("OFFENDER HERE", this.offender_results.results[key].last_name,JSON.stringify(this.offender_results.results[key]))
                    $.post('http://ethical-mdt/viewOffender', JSON.stringify({
		...
		evFile(currentid) {
			    $.post('http://ethical-mdt/evFile', JSON.stringify({
                id: currentid,
            }));
		},
        SaveOffenderChanges() {
            $.post('http://ethical-mdt/saveOffenderChanges', JSON.stringify({
        ...
        ReportSearch() {
            if (this.report_search) {
                // console.log('[JS MDT] ',this.report_search)
                this.report_results.query = this.report_search
                this.warrant_new.report_search = this.report_search
               
                $.post('http://ethical-mdt/performReportSearch', JSON.stringify({
        ...
        SubmitNewReport() {
            if (this.report_new.title && this.report_new.char_id && (Object.keys(this.report_new.charges).length > 0) && this.report_new.incident) {
                $.post('http://ethical-mdt/submitNewReport', JSON.stringify({
        ...
        OpenOffenderDetailsById(id) {
            $.post('http://ethical-mdt/getOffender', JSON.stringify({
                char_id: id
            }));
        ...
        DeleteSelectedReport() {
            $.post('http://ethical-mdt/deleteReport', JSON.stringify({
        ...
        SaveReportEditChanges() {
            $.post('http://ethical-mdt/saveReportChanges', JSON.stringify({
                id: this.report_selected.id,
        ...
        VehicleSearch() {
            if (this.vehicle_search) {
                ...
                $.post('http://ethical-mdt/vehicleSearch', JSON.stringify({
        ...
        OpenVehicleDetails(result) {
            $.post('http://ethical-mdt/getVehicle', JSON.stringify({
                vehicle: result
            }));
        ...
        SubmitNewWarrant() {
            ...
            $.post('http://ethical-mdt/submitNewWarrant', JSON.stringify({
        ...
        DeleteSelectedWarrant() {
            $.post('http://ethical-mdt/deleteWarrant', JSON.stringify({
        ...
        OpenReportById(id) {
            $.post('http://ethical-mdt/getReport', JSON.stringify({
                id: id
            }));
            return;
        },
        ...
        SentencePlayer() {
            ...
            $.post('http://ethical-mdt/sentencePlayer', JSON.stringify({
    ...
    if (document.readyState === "complete") {
        window.addEventListener('message', function(event) {
            if (event.data.type == "enable") {
                ...
                for (let user of mdtApp.offender_results.results) {
                    console.log('first name', user.first_name)
                    $("#search-full-name").html(user.first_name);
                }
                console.log('SEARCH', JSON.stringify(event.data),JSON.stringify(event.data.matches['iswjob']))
            } else if (event.data.type == "returnedOffenderDetails") {
                
                mdtApp.offender_selected = event.data.details;
                console.log("ATLAST OFFERNDER HERE",mdtApp.offender_selected.first_name)
                mdtApp.offender_results.results = false;
                mdtApp.offender_results.query = "";
                mdtApp.offender_changes.notes1 = mdtApp.offender_selected.first_name;
                ...
                mdtApp.offender_search = mdtApp.offender_results.first_name + " " + mdtApp.offender_results.last_name;
                mdtApp.changePage("Search Offenders",mdtApp.offender_results.first_name);
                console.log("OFFERNDER HERE", )
