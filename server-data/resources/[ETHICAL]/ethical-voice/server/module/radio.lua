...
function removePlayerFromRadio(source, currentChannel)
	...
	for player, _ in pairs(radioData[currentChannel]) do
		TriggerClientEvent('ethical-voice:removePlayerFromRadio', player, source)
	end
...
function addPlayerToRadio(source, channel)
	...
	for player, _ in pairs(radioData[channel]) do
		TriggerClientEvent('ethical-voice:addPlayerToRadio', player, source)
	end
	voiceData[source] = voiceData[source] or defaultTable(source)

	voiceData[source].radio = channel
	radioData[channel][source] = false
	TriggerClientEvent('ethical-voice:syncRadioData', source, radioData[channel])
end
...
function setPlayerRadio(source, radioChannel)
	...
	if GetInvokingResource() then
		...
		-- changed
		TriggerClientEvent('ethical-voice:clSetPlayerRadio', source, radioChannel)
	end
	voiceData[source] = voiceData[source] or defaultTable(source)
...
RegisterNetEvent('ethical-voice:setPlayerRadio', function(radioChannel)
	...
	if radioTbl then
		for player, _ in pairs(radioTbl) do
			if player ~= source then
				TriggerClientEvent('ethical-voice:setTalkingOnRadio', player, source, talking)
			end
		end
	end
end
