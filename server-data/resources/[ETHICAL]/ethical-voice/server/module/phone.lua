...
function removePlayerFromCall(source, currentChannel)
    ...
    for player, _ in pairs(callData[currentChannel]) do
        TriggerClientEvent('ethical-voice:removePlayerFromCall', player, source)
    end
...
function addPlayerToCall(source, channel)
    print("[ethical-voice]PLAYER ON PHONE ", source, channel)
    ...
    for player, _ in pairs(callData[channel]) do
        print("[ethical-voice]PLAYER ON PHONE 2 ", source, channel)
        -- don't need to send to the source because they're about to get sync'd!
        if player ~= source then
            TriggerClientEvent('ethical-voice:addPlayerToCall', player, source)
        end
    end
    callData[channel][source] = false
    voiceData[source] = voiceData[source] or defaultTable(source)
    voiceData[source].call = channel
    print("[ethical-voice]syncCallData ", source, json.encode(callData[channel]))
    TriggerClientEvent('ethical-voice:syncCallData', source, callData[channel])
end
...
function setPlayerCall(source, callChannel)
    print('[ethical-voice:setPlayerCall] SetPlayer', source, callChannel)
    ...
    if GetInvokingResource() then
        ...
        -- changed
        TriggerClientEvent('ethical-voice:clSetPlayerCall', source, callChannel)
    end
    voiceData[source] = voiceData[source] or defaultTable(source)
...
RegisterNetEvent('ethical-voice:setPlayerCall', function(callChannel)
...
function setTalkingOnCall(talking)
    ...
    if callTbl then
        for player, _ in pairs(callTbl) do
            if player ~= source then
                TriggerClientEvent('ethical-voice:setTalkingOnCall', player, source, talking)
            end
        end
    end
end
RegisterNetEvent('ethical-voice:setTalkingOnCall', setTalkingOnCall)