ETHICAL.DB = ETHICAL.DB or {}

function ETHICAL.DB.CreateNewPlayer(self, src, callback)
    local hexid = ETHICAL.Util:GetHexId(src)
    callback = callback and callback or function() return end

    local data = {
        hexid = hexid,
        communityid = ETHICAL.Util:HexIdToComId(hexid),
        steamid = ETHICAL.Util:HexIdToSteamId(hexid),
        license = ETHICAL.Util:GetLicense(src),
...
function ETHICAL.DB.CreateNewCharacter(self, src, data2, hexid, ph, callback)
    local hexid = hexid
...
function ETHICAL.DB.PlayerExistsDB(self, src, callback)
    local hexId = ETHICAL.Util:GetHexId(src)
    callback = callback and callback or function() return end
...
function ETHICAL.DB.PhoneNumberExists(self, src, phone_number, callback)
...
function ETHICAL.DB.FetchPlayerData(self, src, callback)
    local hexId = ETHICAL.Util:GetHexId(src)
    callback = callback and callback or function() return end
...
function ETHICAL.DB.FetchCharacterData(self, user, callback)
    callback = callback and callback or function() return end
...
function ETHICAL.DB.DeleteCharacter(self, user, id, callback)
    callback = callback and callback or function() return end
...
function ETHICAL.DB.UpdateCharacterStressLevel(self, user, characterId, newLevel, callback)
    callback = callback and callback or function() return end
    if not user then callback(false,true) return end
...
function ETHICAL.DB.UpdateCharacterDirtyMoney(self, user, characterId, amount, callback)
callback = callback and callback or function() return end
    if not user then callback(false,true) return end
    if not characterId or type(characterId) ~= "number" then callback(false, true) return end
...
function ETHICAL.DB.UpdateCharacterMoney(self, user, characterId, amount, callback)
    callback = callback and callback or function() return end
        if not user then callback(false,true) return end
        if not characterId or type(characterId) ~= "number" then callback(false, true) return end
...
function ETHICAL.DB.UpdateCharacterBank(self, user, characterId, amount, callback)
    callback = callback and callback or function() return end
        if not user then callback(false,true) return end
        if not characterId or type(characterId) ~= "number" then callback(false, true) return end
...
function ETHICAL.DB.UpdateControls(self, src, controlsTable, callback)
    callback = callback and callback or function() return end
        local user = exports["ethical-base"]:getModule("Player"):GetUser(src)
        if not user then callback(false,true) return end
...
function ETHICAL.DB.GetControls(self, src, callback)
    callback = callback and callback or function() return end
    local user = exports["ethical-base"]:getModule("Player"):GetUser(src)
        Citizen.Wait(3000)
        if not user then callback(false, true) return end
...
function ETHICAL.DB.UpdateSettings(self, src, settingsTable, callback)
    callback = callback and callback or function() return end
    local user = exports["ethical-base"]:getModule("Player"):GetUser(src)
        if not user then callback(false,true) return end
...
function ETHICAL.DB.GetSettings(self, src, callback)
    callback = callback and callback or function() return end
    local user = exports["ethical-base"]:getModule("Player"):GetUser(src)
    if not user then callback(false, true) return end
...
-- 3.0
function ETHICAL.DB.Something(self, src, callback)
