ETHICAL.Users = ETHICAL.Users or {}
ETHICAL.Player = ETHICAL.Player or {}

function ETHICAL.Player.GetUser(self, id)
    return ETHICAL.Users[id] and ETHICAL.Users[id] or false
end

function ETHICAL.Player.GetUsers(self)
    local tmp = {}

    for k, v in pairs(ETHICAL.Users) do
        tmp[#tmp+1]= k
    end
...
local function GetUser(user)
    return ETHICAL.Users[user.source]
end
...
local function AddMethod(player)
    ...
    function player.networkVar(self, var, data)
        ...
        TriggerClientEvent("ethical-base:networkVar", GetUser(self):getVar("source"), var, data)
    end
    ...
    function player.alterDirtyMoney(self, amt)
        ...
        ETHICAL.DB:UpdateCharacterDirtyMoney(GetUser(self), characterId, amt, function(updatedMoney, err)
    ...
    function player.alterStressLevel(self, amt)
        ...
        ETHICAL.DB:UpdateCharacterStressLevel(GetUser(self), characterId, amt, function(updatedMoney, err)
            if updatedMoney then
                --We are good here.
            end
        end)
    ...
    function player.resetDirtyMoney(self)
        ...
        ETHICAL.DB:UpdateCharacterDirtyMoney(GetUser(self), characterId, 0, function(updatedMoney, err)
            if updatedMoney then
                --We are good here.
            end
        end)
    ...
    function player.addMoney(self, amt)
        ...
        ETHICAL.DB:UpdateCharacterMoney(GetUser(self), characterId, cash, function(updatedMoney, err) 
            if updatedMoney then
                ...
                exports["ethical-log"]:AddLog("Cash Added", GetUser(self), "Money added to user, amount: " .. tostring(amt))
            end
        end)
    end
        ...
        GetUser(self).character.cash = GetUser(self).character.cash - amt
            ...
            ETHICAL.DB:UpdateCharacterMoney(GetUser(self), characterId, cash, function(updatedMoney, err) 
                if updatedMoney then
                    ...
                    exports["ethical-log"]:AddLog("Cash Removed", GetUser(self), "Money removed from user, amount: " .. tostring(amt))
                end
            end)
    end
    ...
    function player.removeBank(self, amt)
        ...
        ETHICAL.DB:UpdateCharacterBank(GetUser(self), characterId, bank, function(updatedMoney, err) 
            if updatedMoney then
                ...
                exports["ethical-log"]:AddLog("Bank Removed", GetUser(self), "Bank removed from user, amount: " .. tostring(amt))
            end
        end)
    end
    ...
    function player.addBank(self, amt)
        ...
        ETHICAL.DB:UpdateCharacterBank(GetUser(self), characterId, bank, function(updatedMoney, err) 
            if updatedMoney then
                ...
                exports["ethical-log"]:AddLog("Bank Added", GetUser(self), "Bank added to user, amount: " .. tostring(amt))
            end
        end)
    end
    ...
    local function CreatePlayer(src)
        ...
        self.hexid = ETHICAL.Util:GetHexId(src)
        ...
        self.comid = ETHICAL.Util:HexIdToComId(self.hexid)
        self.steamid = ETHICAL.Util:HexIdToSteamId(self.hexid)
        self.license = ETHICAL.Util:GetLicense(src)
        ...
        ETHICAL.Users[src] = methods

        return methods
    end


function ETHICAL.Player.CreatePlayer(self, src, recrate)
    if recreate then ETHICAL.Users[src] = nil end
    
    if ETHICAL.Users[src] then return ETHICAL.Users[src] end

    return CreatePlayer(src)
end
local pos = {}
RegisterServerEvent('ethical-base:updatecoords')
AddEventHandler('ethical-base:updateCoords', function(x,y,z)
    local src = source
    pos[src] = {x,y,z}
end)
...
AddEventHandler("playerDropped", function(reason)
    ...
    pos[src] = nil
    if user then exports["ethical-log"]:AddLog("Player Left", user, user:getVar("name").." Has left the server") end

    ETHICAL.Users[src] = nil

    TriggerEvent('ethical-base:playerDropped', src, user)
end)
