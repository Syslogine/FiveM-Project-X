ETHICAL.DataControls = ETHICAL.DataControls or {}
ETHICAL.Controls = ETHICAL.Controls or {}
ETHICAL.Controls.EventHolder = {}


ETHICAL.Controls.Current = {}
-- Current bind name and keys
ETHICAL.Controls.Default = {
  ["tokoptt"] = "CAPS",
  ["loudSpeaker"] = "-",
  ["distanceChange"] = "G",
  ["tokoToggle"] = "LEFTCTRL",
...
ETHICAL.Controls.secondaryBinds = {}

ETHICAL.Controls.Distcheck = {}
ETHICAL.Controls.Distcheck.use = {
	-- create fake id
...
ETHICAL.Controls.Distcheck.useSecondary = {
...
ETHICAL.Controls.Distcheck.useThird = {
	-- hospital garage
...
function ETHICAL.DataControls.getBindTable()

	local i = 1
	local controlTable = {}
	for k,v in pairs(ETHICAL.Controls.Current) do
		controlTable[i] = {k,v}
		i = i+1
	end
...
function ETHICAL.DataControls.encodeSetBindTable(self, bindTable)
	...
	ETHICAL.DataControls.setBindTable(controlTable,true)
end

function ETHICAL.DataControls.toUpper(table)
...
function ETHICAL.DataControls.setBindTable(controlTable,shouldSend)
	if controlTable == nil then
		ETHICAL.Controls.Current  = ETHICAL.DataControls.toUpper(ETHICAL.Controls.Default) 
		ETHICAL.DataControls.setSecondaryBindTable(ETHICAL.Controls.Current)
		TriggerServerEvent('ethical-base:sv:player_control_set',ETHICAL.Controls.Current)
		ETHICAL.DataControls.checkForMissing()
	else
		if shouldSend then 
			ETHICAL.Controls.Current = ETHICAL.DataControls.toUpper(controlTable)
			ETHICAL.DataControls.setSecondaryBindTable(ETHICAL.Controls.Current)
			TriggerServerEvent('ethical-base:sv:player_control_set',ETHICAL.Controls.Current)
			ETHICAL.DataControls.checkForMissing()
		else
	...
	TriggerEvent("event:control:update",ETHICAL.DataControls.getTableInKeyNumbers())
end

function ETHICAL.DataControls.setSecondaryBindTable(bindtable)
	...
	for k,v in pairs(bindtable) do
		...
		if string.match(keyString, "+")  then
			...
			local valid = true
			for i=1,#ETHICAL.Controls.secondaryBinds  do
				if ETHICAL.Controls.secondaryBinds[i] and ETHICAL.Controls.secondaryBinds[i][1] == control1 then valid = false end
			end

			if valid then 
				ETHICAL.Controls.secondaryBinds[i] = {}
				ETHICAL.Controls.secondaryBinds[i][1] = control1
				ETHICAL.Controls.secondaryBinds[i][2] = control2
				i = i + 1
			end
...
function ETHICAL.DataControls.getTableInKeyNumbers()
...
function ETHICAL.DataControls.getBindByID(bindID)
	local found = "none"
	for k,v in pairs(ETHICAL.Controls.Current) do
		if k == bindID then
			found = v
...
function ETHICAL.DataControls.distanceCall(distcheckName)
	local distanceTable = ETHICAL.Controls.Distcheck[distcheckName]
	local found = -1
	...
	if found ~= -1 then
		...
		if ETHICAL.Controls.EventHolder[key] then
			hasTimer = (GetGameTimer()-ETHICAL.Controls.EventHolder[key])
		end
		if  hasTimer >= distanceTable[found].maxwait then
			ETHICAL.Controls.EventHolder[key] = GetGameTimer();
...
function ETHICAL.DataControls.checkForMissing()
	local isMissing = false

	for j,h in pairs(ETHICAL.Controls.Default) do
		if ETHICAL.Controls.Current[j] == nil then
			isMissing = true
			ETHICAL.Controls.Current[j] = h
		end
	end
...
function ETHICAL.DataControls.validEvent(event,id,eventID)
	local v = ETHICAL.Controls.events["general"][eventID]
	local hasTimer = 999999
	local eventKey = event.."_"..id

	if ETHICAL.Controls.EventHolder[eventKey] then
		hasTimer = (GetGameTimer()-ETHICAL.Controls.EventHolder[eventKey])
	end

	if ETHICAL.Controls.EventHolder[eventKey] and hasTimer ~= 999999 then
		if hasTimer >= v.maxwait then
			ETHICAL.Controls.EventHolder[eventKey] = GetGameTimer();
...
Citizen.CreateThread(function()
	while true do
		...
		if ETHICAL.Controls.Current["tokoptt"] then
			for i=0,#ETHICAL.Controls.events["general"] do
				local v = ETHICAL.Controls.events["general"][i]
				local keyString = ETHICAL.Controls.Current[v["bind"]]
				local key = Keys[keyString]

				if key == 199 then
					if IsDisabledControlJustReleased(1,key) then
						local isConflicted = -1
						for i=0,#ETHICAL.Controls.secondaryBinds do
							if IsControlPressed(1,ETHICAL.Controls.secondaryBinds[i][1]) or IsDisabledControlPressed(1,ETHICAL.Controls.secondaryBinds[i][1]) then
								isConflicted = ETHICAL.Controls.secondaryBinds[i][1]
							end
						end
						...
						if isConflicted ~= -1 then
							...
							if IsControlPressed(1,isConflicted) or IsDisabledControlPressed(1,isConflicted) then
								...
								ETHICAL.DataControls.validEvent(eventBind,idEvent,i)
							else
								ETHICAL.DataControls.validEvent(v.event,v.id,i)
							end
						else
					...
					if IsControlJustPressed(1,key) and not IsControlJustPressed(1,0) and not IsControlJustPressed(1,nil) then
						if GetLastInputMethod(0) then
							...
							if validTrigger then
								local isConflicted = -1
								for i=0,#ETHICAL.Controls.secondaryBinds do
									if IsControlPressed(1,ETHICAL.Controls.secondaryBinds[i][2]) or IsDisabledControlPressed(1,ETHICAL.Controls.secondaryBinds[i][2]) then
										isConflicted = ETHICAL.Controls.secondaryBinds[i][2]
									end
								end
...
Citizen.CreateThread(function()
	while true do
		...
		if ETHICAL.Controls.Current["tokoptt"] then
			for i=0,#ETHICAL.Controls.events["general-dist"] do
				local v = ETHICAL.Controls.events["general-dist"][i]
				local key = Keys[ETHICAL.Controls.Current[v["bind"]]]
				local hasTimer = 99999;
				...
				if IsControlJustPressed(1,key) and hasTimer > 500 then	
					...
					ETHICAL.DataControls.distanceCall(v["distanceName"])
				end	
			end
		end
		Wait(5)
...
RegisterNetEvent("ethical-base:cl:player_control")
AddEventHandler("ethical-base:cl:player_control", function(controlTable)
	ETHICAL.DataControls.setBindTable(controlTable,false)
end)
...
RegisterNetEvent('ethical-base:setcontrols')
AddEventHandler('ethical-base:setcontrols', function()
	TriggerServerEvent('ethical-base:sv:player_controls')
end)
