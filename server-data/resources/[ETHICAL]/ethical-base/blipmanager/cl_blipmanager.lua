ETHICAL.BlipManager = ETHICAL.BlipManager or {}
ETHICAL.Blips = ETHICAL.Blips or {}

function ETHICAL.BlipManager.CreateBlip(self, id, data)
    ...
    ETHICAL.Blips[id] = {blip = blip, data = data}
end

function ETHICAL.BlipManager.RemoveBlip(self, id)
    local blip = ETHICAL.Blips[id]
    if blip then RemoveBlip(blip.blip) end
    ETHICAL.Blips[id] = nil
end

function ETHICAL.BlipManager.HideBlip(self, id, toggle)
    local blip = ETHICAL.Blips[id]
    if not blip then return end
    if toggle then SetBlipAlpha(blip, 0) else SetBlipAlpha(blip, 255) end
end

function ETHICAL.BlipManager.GetBlip(self, id)
    local blip = ETHICAL.Blips[id]
