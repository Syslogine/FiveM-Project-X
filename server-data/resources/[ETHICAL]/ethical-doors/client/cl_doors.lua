...
AddEventHandler( 'cell:doors', function(num)
    ...
    TriggerServerEvent("ethical-doors:alterlockstate",tonumber(num))
end)
        ...
        if curClosestNum ~= 0 and targetDoor ~= nil then
            ...
            if currentTargetDist < drawdist then
                if isKeyDoor(curClosestNum) then
                    ...
                    exports['ethical-interaction']:showInteraction("["..Controlkey["generalUse"][2].."] " .. closestString .. ""  , color )
                        ...
                        if shortcheck > 2.2 and isKeyDoor(curClosestNum) then
                            ...
                            if targetDoor["lock"] == 0 then
                                ...
                                while active do
                                    ...
                                    exports['ethical-interaction']:showInteraction('Locking'  , 'error' )
                                ...
                                while active do
                                    Citizen.Wait(1)
                                    exports['ethical-interaction']:showInteraction('Unlocking')
                            ...
                            TriggerServerEvent("ethical-doors:alterlockstate",curClosestNum)
                        end
...
function OpenCheck(curClosestNum)
  ...
  local job = exports['isPed']:isPed('myjob')
    ...
    if (job == "ems" or job == "doctor" or job == "police") and (curClosestNum >= 18 and curClosestNum <= 32) then
        return true
    end
    ...
    if (job == "tuner_carshop" or job == "tuner_shop" or job == "police") and (curClosestNum >= 97 and curClosestNum <= 100) then
        return true
    end

    if (job == "DriftSchool" or job == "police") and (curClosestNum >= 105 and curClosestNum <= 109) then
        return true
    end
    ...
    if (job == "FIB" or job == "police") and (curClosestNum >= 109 and curClosestNum <= 111) then
        return true
    end

    if (job == "Gallery" or job == "police") and (curClosestNum >= 111 and curClosestNum <= 130) then
        return true
    end

    if (job == "WhiteWidow" or job == "police") and (curClosestNum >= 130 and curClosestNum <= 133) then
        return true
    end

    if (job == "BurgerShot" or job == "OffBurgerShot" or job == "police") and (curClosestNum >= 134 and curClosestNum <= 137) then
        return true
    end
end
