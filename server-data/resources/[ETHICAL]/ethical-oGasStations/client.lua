...
AddEventHandler("RefuelCarServerReturn",function()
    ...
    refillVehicle()
    local finished = exports["ethical-taskbar"]:taskBar(timer,"Refueling")
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if IsVehicleEngineOn(veh) then          
            ...
            if not exports["ethical-inventory"]:hasEnoughOfItem("watch",1,false) then
                compass_on = false
            end
        end
    end
...
AddEventHandler('carHud:compass', function(table)
    if exports["ethical-inventory"]:hasEnoughOfItem("watch",1,false) then
