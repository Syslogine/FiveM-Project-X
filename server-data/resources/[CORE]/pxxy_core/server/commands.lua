PXXY.RegisterCommand(
    { "setcoords", "tp" },
    "admin",
    function(xPlayer, args)
        xPlayer.setCoords({ x = args.x, y = args.y, z = args.z })
        if Config.AdminLogging then
            PXXY.DiscordLogFields("UserActions", "Set Coordinates /setcoords Triggered!", "pink", {
...
PXXY.RegisterCommand(
    ...
    function(xPlayer, args, showError)
        if not PXXY.DoesJobExist(args.job, args.grade) then
            return showError(TranslateCap("command_setjob_invalid"))
        end

        args.playerId.setJob(args.job, args.grade)
        if Config.AdminLogging then
            PXXY.DiscordLogFields("UserActions", "Set Job /setjob Triggered!", "pink", {
                { name = "Player", value = xPlayer and xPlayer.name or "Server Console", inline = true },
                { name = "ID", value = xPlayer and xPlayer.source or "Unknown ID", inline = true },
...
PXXY.RegisterCommand(
    ...
    function(xPlayer, args, showError)
        ...
        if Config.AdminLogging then
            PXXY.DiscordLogFields("UserActions", "Spawn Car /car Triggered!", "pink", {
                { name = "Player", value = xPlayer and xPlayer.name or "Server Console", inline = true },
                { name = "ID", value = xPlayer and xPlayer.source or "Unknown ID", inline = true },
        ...
        PXXY.OneSync.SpawnVehicle(args.car, playerCoords, playerHeading, upgrades, function(networkId)
            if networkId then
...
PXXY.RegisterCommand(
    ...
    function(xPlayer, args)
        ...
        local Vehicles = PXXY.OneSync.GetVehiclesInArea(GetEntityCoords(GetPlayerPed(xPlayer.source)), tonumber(args.radius) or 5.0)
        for i = 1, #Vehicles do
        ...
        if Config.AdminLogging then
            PXXY.DiscordLogFields("UserActions", "Delete Vehicle /dv Triggered!", "pink", {
                { name = "Player", value = xPlayer and xPlayer.name or "Server Console", inline = true },
                { name = "ID", value = xPlayer and xPlayer.source or "Unknown ID", inline = true },
            })
        end
...
PXXY.RegisterCommand(
    ...
    function(xPlayer, args, showError)
        ...
        xTarget.triggerEvent("pxxy:repairPedVehicle")
        ...
        if Config.AdminLogging then
            PXXY.DiscordLogFields("UserActions", "Fix Vehicle /fix Triggered!", "pink", {
                { name = "Player", value = xPlayer and xPlayer.name or "Server Console", inline = true },
                { name = "ID", value = xPlayer and xPlayer.source or "Unknown ID", inline = true },
...
PXXY.RegisterCommand(
    ...
    function(xPlayer, args, showError)
        ...
        if Config.AdminLogging then
            PXXY.DiscordLogFields("UserActions", "Set Account Money /setaccountmoney Triggered!", "pink", {
                { name = "Player", value = xPlayer and xPlayer.name or "Server Console", inline = true },
                { name = "ID", value = xPlayer and xPlayer.source or "Unknown ID", inline = true },
                { name = "Target", value = args.playerId.name, inline = true },
...
PXXY.RegisterCommand(
    ...
    function(xPlayer, args, showError)
        ...
        if Config.AdminLogging then
            PXXY.DiscordLogFields("UserActions", "Give Account Money /giveaccountmoney Triggered!", "pink", {
                { name = "Player", value = xPlayer and xPlayer.name or "Server Console", inline = true },
                { name = "ID", value = xPlayer and xPlayer.source or "Unknown ID", inline = true },
                { name = "Target", value = args.playerId.name, inline = true },
                { name = "Account", value = args.account, inline = true },
...
PXXY.RegisterCommand(
    ...
    function(xPlayer, args, showError)
        ...
        if Config.AdminLogging then
            PXXY.DiscordLogFields("UserActions", "Remove Account Money /removeaccountmoney Triggered!", "pink", {
                { name = "Player", value = xPlayer and xPlayer.name or "Server Console", inline = true },
                { name = "ID", value = xPlayer and xPlayer.source or "Unknown ID", inline = true },
                { name = "Target", value = args.playerId.name, inline = true },
                { name = "Account", value = args.account, inline = true },
...
if not Config.OxInventory then
    PXXY.RegisterCommand(
        ...
        function(xPlayer, args)
            ...
            if Config.AdminLogging then
                PXXY.DiscordLogFields("UserActions", "Give Item /giveitem Triggered!", "pink", {
                    { name = "Player", value = xPlayer and xPlayer.name or "Server Console", inline = true },
                    { name = "ID", value = xPlayer and xPlayer.source or "Unknown ID", inline = true },
                    { name = "Target", value = args.playerId.name, inline = true },
    ...
    PXXY.RegisterCommand(
        ...
        function(xPlayer, args, showError)
            ...
            if Config.AdminLogging then
                PXXY.DiscordLogFields("UserActions", "Give Weapon /giveweapon Triggered!", "pink", {
                    { name = "Player", value = xPlayer and xPlayer.name or "Server Console", inline = true },
                    { name = "ID", value = xPlayer and xPlayer.source or "Unknown ID", inline = true },
                    { name = "Target", value = args.playerId.name, inline = true },
    ...
    PXXY.RegisterCommand(
        ...
        function(xPlayer, args, showError)
            ...
            if Config.AdminLogging then
                PXXY.DiscordLogFields("UserActions", "Give Ammunition /giveammo Triggered!", "pink", {
                    { name = "Player", value = xPlayer and xPlayer.name or "Server Console", inline = true },
                    { name = "ID", value = xPlayer and xPlayer.source or "Unknown ID", inline = true },
                    { name = "Target", value = args.playerId.name, inline = true },
                    { name = "Weapon", value = args.weapon, inline = true },
    ...
    PXXY.RegisterCommand(
        ...
        function(xPlayer, args, showError)
            if args.playerId.hasWeapon(args.weaponName) then
                local component = PXXY.GetWeaponComponent(args.weaponName, args.componentName)
                ...
                if component then
                    if args.playerId.hasWeaponComponent(args.weaponName, args.componentName) then
                        ...
                        if Config.AdminLogging then
                            PXXY.DiscordLogFields("UserActions", "Give Weapon Component /giveweaponcomponent Triggered!", "pink", {
                                { name = "Player", value = xPlayer and xPlayer.name or "Server Console", inline = true },
                                { name = "ID", value = xPlayer and xPlayer.source or "Unknown ID", inline = true },
                                { name = "Target", value = args.playerId.name, inline = true },
...
PXXY.RegisterCommand({ "clear", "cls" }, "user", function(xPlayer)
    xPlayer.triggerEvent("chat:clear")
end, false, { help = TranslateCap("command_clear") })

PXXY.RegisterCommand({ "clearall", "clsall" }, "admin", function(xPlayer)
    TriggerClientEvent("chat:clear", -1)
    if Config.AdminLogging then
        PXXY.DiscordLogFields("UserActions", "Clear Chat /clearall Triggered!", "pink", {
            { name = "Player", value = xPlayer and xPlayer.name or "Server Console", inline = true },
            { name = "ID", value = xPlayer and xPlayer.source or "Unknown ID", inline = true },
        })
    end
...
PXXY.RegisterCommand("refreshjobs", "admin", function()
    PXXY.RefreshJobs()
end, true, { help = TranslateCap("command_clearall") })
...
if not Config.OxInventory then
    PXXY.RegisterCommand(
        ...
        function(xPlayer, args)
            ...
            TriggerEvent("pxxy:playerInventoryCleared", args.playerId)
            if Config.AdminLogging then
                PXXY.DiscordLogFields("UserActions", "Clear Inventory /clearinventory Triggered!", "pink", {
                    { name = "Player", value = xPlayer and xPlayer.name or "Server Console", inline = true },
                    { name = "ID", value = xPlayer and xPlayer.source or "Unknown ID", inline = true },
                    { name = "Target", value = args.playerId.name, inline = true },
                })
    ...
    PXXY.RegisterCommand(
        ...
        function(xPlayer, args)
            ...
            TriggerEvent("pxxy:playerLoadoutCleared", args.playerId)
            if Config.AdminLogging then
                PXXY.DiscordLogFields("UserActions", "/clearloadout Triggered!", "pink", {
                    { name = "Player", value = xPlayer and xPlayer.name or "Server Console", inline = true },
                    { name = "ID", value = xPlayer and xPlayer.source or "Unknown ID", inline = true },
                    { name = "Target", value = args.playerId.name, inline = true },
                })
...
PXXY.RegisterCommand(
    ...
    function(xPlayer, args)
        ...
        if Config.AdminLogging then
            PXXY.DiscordLogFields("UserActions", "/setgroup Triggered!", "pink", {
                { name = "Player", value = xPlayer and xPlayer.name or "Server Console", inline = true },
                { name = "ID", value = xPlayer and xPlayer.source or "Unknown ID", inline = true },
                { name = "Target", value = args.playerId.name, inline = true },
...
PXXY.RegisterCommand("tpm", "admin", function(xPlayer)
    xPlayer.triggerEvent("pxxy:tpm")
    if Config.AdminLogging then
        PXXY.DiscordLogFields("UserActions", "Admin Teleport /tpm Triggered!", "pink", {
            { name = "Player", value = xPlayer and xPlayer.name or "Server Console", inline = true },
            { name = "ID", value = xPlayer and xPlayer.source or "Unknown ID", inline = true },
        })
    end
...
PXXY.RegisterCommand(
    ...
    function(xPlayer, args)
        ...
        if Config.AdminLogging then
            PXXY.DiscordLogFields("UserActions", "Admin Teleport /goto Triggered!", "pink", {
                { name = "Player", value = xPlayer and xPlayer.name or "Server Console", inline = true },
                { name = "ID", value = xPlayer and xPlayer.source or "Unknown ID", inline = true },
                { name = "Target", value = args.playerId.name, inline = true },
...
PXXY.RegisterCommand(
    ...
    function(xPlayer, args)
        ...
        if Config.AdminLogging then
            PXXY.DiscordLogFields("UserActions", "Admin Teleport /bring Triggered!", "pink", {
                { name = "Player", value = xPlayer and xPlayer.name or "Server Console", inline = true },
                { name = "ID", value = xPlayer and xPlayer.source or "Unknown ID", inline = true },
                { name = "Target", value = args.playerId.name, inline = true },
                { name = "Target Coords", value = targetCoords, inline = true },
...
PXXY.RegisterCommand(
    ...
    "admin",
    function(xPlayer, args)
        args.playerId.triggerEvent("pxxy:killPlayer")
        if Config.AdminLogging then
            PXXY.DiscordLogFields("UserActions", "Kill Command /kill Triggered!", "pink", {
                { name = "Player", value = xPlayer and xPlayer.name or "Server Console", inline = true },
                { name = "ID", value = xPlayer and xPlayer.source or "Unknown ID", inline = true },
                { name = "Target", value = args.playerId.name, inline = true },
            })
...
PXXY.RegisterCommand(
    ...
    "admin",
    function(xPlayer, args)
        args.playerId.triggerEvent("pxxy:freezePlayer", "freeze")
        if Config.AdminLogging then
            PXXY.DiscordLogFields("UserActions", "Admin Freeze /freeze Triggered!", "pink", {
                { name = "Player", value = xPlayer and xPlayer.name or "Server Console", inline = true },
                { name = "ID", value = xPlayer and xPlayer.source or "Unknown ID", inline = true },
                { name = "Target", value = args.playerId.name, inline = true },
            })
...
PXXY.RegisterCommand(
    ...
    "admin",
    function(xPlayer, args)
        args.playerId.triggerEvent("pxxy:freezePlayer", "unfreeze")
        if Config.AdminLogging then
            PXXY.DiscordLogFields("UserActions", "Admin UnFreeze /unfreeze Triggered!", "pink", {
                { name = "Player", value = xPlayer and xPlayer.name or "Server Console", inline = true },
                { name = "ID", value = xPlayer and xPlayer.source or "Unknown ID", inline = true },
                { name = "Target", value = args.playerId.name, inline = true },
            })
...
PXXY.RegisterCommand("noclip", "admin", function(xPlayer)
    xPlayer.triggerEvent("pxxy:noclip")
    if Config.AdminLogging then
        PXXY.DiscordLogFields("UserActions", "Admin NoClip /noclip Triggered!", "pink", {
            { name = "Player", value = xPlayer and xPlayer.name or "Server Console", inline = true },
            { name = "ID", value = xPlayer and xPlayer.source or "Unknown ID", inline = true },
        })
    end
...
PXXY.RegisterCommand("players", "admin", function()
    local xPlayers = PXXY.GetExtendedPlayers() -- Returns all xPlayers
