function PXXY.Trace(msg)
    if Config.EnableDebug then
        print(("[^2TRACE^7] %s^7"):format(msg))
    end
end

function PXXY.RegisterCommand(name, group, cb, allowConsole, suggestion)
    if type(name) == "table" then
        for _, v in ipairs(name) do
            PXXY.RegisterCommand(v, group, cb, allowConsole, suggestion)
        end
    ...
    RegisterCommand(name, function(playerId, args)
        ...
        if not command.allowConsole and playerId == 0 then
            ...
            local xPlayer, error = PXXY.Players[playerId], nil
                        ...
                        if v.type then
                            ...
                            elseif v.type == "player" or v.type == "playerId" then
                                ...
                                if targetPlayer then
                                    local xTargetPlayer = PXXY.GetPlayerFromId(targetPlayer)
                            ...
                            elseif v.type == "item" then
                                if PXXY.Items[args[k]] then
                                    newArgs[v.name] = args[k]
                                else
                                    error = TranslateCap("commanderror_invaliditem")
                                end
                            elseif v.type == "weapon" then
                                if PXXY.GetWeapon(args[k]) then
    ...
    MySQL.prepare(
        ...
        function(affectedRows)
            if affectedRows == 1 then
                ...
                TriggerEvent("pxxy:playerSaved", xPlayer.playerId, xPlayer)
            end
            if cb then
                cb()
            end
...
function Core.SavePlayers(cb)
    local xPlayers <const> = PXXY.Players
    ...
    MySQL.prepare(
        ...
        function(results)
            ...
            print(("[^2INFO^7] Saved ^5%s^7 %s over ^5%s^7 ms"):format(#parameters, #parameters > 1 and "players" or "player", PXXY.Math.Round((os.time() - startTime) / 1000000, 2)))
        end
    )
end

PXXY.GetPlayers = GetPlayers
...
function PXXY.GetExtendedPlayers(key, val)
    local xPlayers = {}
    if type(val) == "table" then
        for _, v in pairs(PXXY.Players) do
...
function PXXY.GetNumPlayers(key, val)
    if not key then
        return #GetPlayers()
    end
    ...
    if type(val) == "table" then
        ...
        if key == "job" then
            for _, v in ipairs(val) do
                numPlayers[v] = (PXXY.JobsPlayerCount[v] or 0)
            end
            return numPlayers
        end

        local filteredPlayers = PXXY.GetExtendedPlayers(key, val)
    ...
    if key == "job" then
        return (PXXY.JobsPlayerCount[val] or 0)
    end

    return #PXXY.GetExtendedPlayers(key, val)
end

function PXXY.GetPlayerFromId(source)
    return PXXY.Players[tonumber(source)]
end

function PXXY.GetPlayerFromIdentifier(identifier)
    return Core.playersByIdentifier[identifier]
end

function PXXY.GetIdentifier(playerId)
...
function PXXY.GetVehicleType(model, player, cb)
    ...
    PXXY.TriggerClientCallback(player, "pxxy:GetVehicleType", function(vehicleType)
        Core.vehicleTypesByModel[model] = vehicleType
        cb(vehicleType)
    end, model)
end

function PXXY.DiscordLog(name, title, color, message)
...
function PXXY.DiscordLogFields(name, title, color, fields)
    local webHook = Config.DiscordLogs.Webhooks[name] or Config.DiscordLogs.Webhooks.default
    local embedData = {
        {
            ["title"] = title,
...
--- @param label string
--- @param grades table
function PXXY.CreateJob(name, label, grades)
    if not name then
...
function PXXY.RefreshJobs()
    ...
    for _, v in pairs(Jobs) do
        if PXXY.Table.SizeOf(v.grades) == 0 then
            Jobs[v.name] = nil
    ...
    if not Jobs then
        ...
        PXXY.Jobs["unemployed"] = { label = "Unemployed", grades = { ["0"] = { grade = 0, label = "Unemployed", salary = 200, skin_male = {}, skin_female = {} } } }
    else
        PXXY.Jobs = Jobs
    end
end

function PXXY.RegisterUsableItem(item, cb)
    Core.UsableItemsCallbacks[item] = cb
end

function PXXY.UseItem(source, item, ...)
    if PXXY.Items[item] then
        ...
        if itemCallback then
            ...
            if not success then
                return result and print(result) or print(('[^3WARNING^7] An error occured when using item ^5"%s"^7! This was not caused by PXXY.'):format(item))
            end
        end
    else
...
function PXXY.RegisterPlayerFunctionOverrides(index, overrides)
    Core.PlayerFunctionOverrides[index] = overrides
end

function PXXY.SetPlayerFunctionOverride(index)
...
function PXXY.GetItemLabel(item)
    ...
    if PXXY.Items[item] then
        return PXXY.Items[item].label
    else
        print(("[^3WARNING^7] Attemting to get invalid Item -> ^5%s^7"):format(item))
    end
end

function PXXY.GetJobs()
    return PXXY.Jobs
end

function PXXY.GetUsableItems()
    local Usables = {}
...
if not Config.OxInventory then
    function PXXY.CreatePickup(itemType, name, count, label, playerId, components, tintIndex, coords)
        local pickupId = (Core.PickupId == 65635 and 0 or Core.PickupId + 1)
        local xPlayer = PXXY.Players[playerId]
        ...
        TriggerClientEvent("pxxy:createPickup", -1, pickupId, label, coords, itemType, name, components, tintIndex)
        Core.PickupId = pickupId
    end
end

function PXXY.DoesJobExist(job, grade)
    grade = tostring(grade)

    if job and grade then
        if PXXY.Jobs[job] and PXXY.Jobs[job].grades[grade] then
            return true
        end
    end
