        ...
        return function(accountName, money, reason)
            ...
            if money >= 0 then
                ...
                if account then
                    ...
                    TriggerEvent("pxxy:setAccountMoney", self.source, accountName, money, reason)
        ...
        return function(accountName, money, reason)
            ...
            if money > 0 then
                ...
                if account then
                    ...
                    self.triggerEvent("pxxy:setAccountMoney", account)
                    TriggerEvent("pxxy:addAccountMoney", self.source, accountName, money, reason)
                    if Inventory.accounts[accountName] then
        ...
        return function(accountName, money, reason)
            ...
            if money > 0 then
                ...
                if account then
                    ...
                    self.accounts[account.index].money = self.accounts[account.index].money - money
                    self.triggerEvent("pxxy:setAccountMoney", account)
                    TriggerEvent("pxxy:removeAccountMoney", self.source, accountName, money, reason)
                    if Inventory.accounts[accountName] then
        ...
        return function(weight, maxWeight, items, money)
            ...
            if money then
                for accountName, amount in pairs(money) do
                    ...
                    if account and PXXY.Math.Round(account.money) ~= amount then
                        account.money = amount
                        self.triggerEvent("pxxy:setAccountMoney", account)
                        TriggerEvent("pxxy:setAccountMoney", self.source, accountName, amount, "Sync account with item")
                    end
                end
            end
        end
