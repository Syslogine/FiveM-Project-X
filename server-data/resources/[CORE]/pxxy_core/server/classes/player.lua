...
function CreateExtendedPlayer(playerId, identifier, group, accounts, inventory, weight, job, loadout, name, coords, metadata)
    ...
    function self.setAccountMoney(accountName, money, reason)
        ...
        if money >= 0 then
            ...
            if account then
                money = account.round and PXXY.Math.Round(money) or money
                self.accounts[account.index].money = money

                self.triggerEvent("pxxy:setAccountMoney", account)
                _TriggerEvent("pxxy:setAccountMoney", self.source, accountName, money, reason)
            else
    ...
    function self.addAccountMoney(accountName, money, reason)
        ...
        if money > 0 then
            ...
            if account then
                ...
                self.triggerEvent("pxxy:setAccountMoney", account)
                _TriggerEvent("pxxy:addAccountMoney", self.source, accountName, money, reason)
            else
    ...
    function self.removeAccountMoney(accountName, money, reason)
        ...
        if money > 0 then
            ...
            if account then
                ...
                self.triggerEvent("pxxy:setAccountMoney", account)
                _TriggerEvent("pxxy:removeAccountMoney", self.source, accountName, money, reason)
            else
                print(("[^1ERROR^7] Tried To Set Add To Invalid Account ^5%s^0 For Player ^5%s^0!"):format(accountName, self.playerId))
            end
        else
    ...
    function self.addInventoryItem(itemName, count)
        ...
        if item then
            count = PXXY.Math.Round(count)
            item.count = item.count + count
            self.weight = self.weight + (item.weight * count)

            _TriggerEvent("pxxy:onAddInventoryItem", self.source, item.name, item.count)
            self.triggerEvent("pxxy:addInventoryItem", item.name, item.count)
        end
    end
        ...
        if item then
            ...
            if count > 0 then
                ...
                if newCount >= 0 then
                    ...
                    _TriggerEvent("pxxy:onRemoveInventoryItem", self.source, item.name, item.count)
                    self.triggerEvent("pxxy:removeInventoryItem", item.name, item.count)
                end
            else
    ...
    function self.canCarryItem(itemName, count)
        if PXXY.Items[itemName] then
            local currentWeight, itemWeight = self.weight, PXXY.Items[itemName].weight
    ...
    function self.canSwapItem(firstItem, firstItemCount, testItem, testItemCount)
        ...
        if firstItemObject.count >= firstItemCount then
            local weightWithoutFirstItem = PXXY.Math.Round(self.weight - (firstItemObject.weight * firstItemCount))
            local weightWithTestItem = PXXY.Math.Round(weightWithoutFirstItem + (testItemObject.weight * testItemCount))
    ...
    function self.setMaxWeight(newWeight)
        ...
        self.triggerEvent("pxxy:setMaxWeight", self.maxWeight)
    end
    ...
    function self.setJob(newJob, grade)
        ...
        if not PXXY.DoesJobExist(newJob, grade) then
            return print(("[pxxy_core] [^3WARNING^7] Ignoring invalid ^5.setJob()^7 usage for ID: ^5%s^7, Job: ^5%s^7"):format(self.source, job))
        end

        local jobObject, gradeObject = PXXY.Jobs[newJob], PXXY.Jobs[newJob].grades[grade]
        ...
        TriggerEvent("pxxy:setJob", self.source, self.job, lastJob)
        self.triggerEvent("pxxy:setJob", self.job, lastJob)
    ...
    function self.addWeapon(weaponName, ammo)
        if not self.hasWeapon(weaponName) then
            local weaponLabel <const> = PXXY.GetWeaponLabel(weaponName)
            ...
            self.triggerEvent("pxxy:addInventoryItem", weaponLabel, false, true)
        end
    end
    ...
    function self.addWeaponComponent(weaponName, weaponComponent)
        ...
        if weapon then
            local component = PXXY.GetWeaponComponent(weaponName, weaponComponent)

            if component then
                if not self.hasWeaponComponent(weaponName, weaponComponent) then
                    self.loadout[loadoutNum].components[#self.loadout[loadoutNum].components + 1] = weaponComponent
                    local componentHash = PXXY.GetWeaponComponent(weaponName, weaponComponent).hash
                    GiveWeaponComponentToPed(GetPlayerPed(self.source), joaat(weaponName), componentHash)
                    self.triggerEvent("pxxy:addInventoryItem", component.label, false, true)
                end
            end
        end
    end
    ...
    function self.setWeaponTint(weaponName, weaponTintIndex)
        ...
        if weapon then
            local _, weaponObject <const> = PXXY.GetWeapon(weaponName)

            if weaponObject.tints and weaponObject.tints[weaponTintIndex] then
                self.loadout[loadoutNum].tintIndex = weaponTintIndex
                self.triggerEvent("pxxy:setWeaponTint", weaponName, weaponTintIndex)
                self.triggerEvent("pxxy:addInventoryItem", weaponObject.tints[weaponTintIndex], false, true)
            end
        end
    end
    ...
    function self.removeWeapon(weaponName)
        ...
        if weaponLabel then
            self.triggerEvent("pxxy:removeInventoryItem", weaponLabel, false, true)
        end
    end
    ...
    function self.removeWeaponComponent(weaponName, weaponComponent)
        ...
        if weapon then
            ---@type table
            local component <const> = PXXY.GetWeaponComponent(weaponName, weaponComponent)
            ...
            if component then
                if self.hasWeaponComponent(weaponName, weaponComponent) then
                    ...
                    self.triggerEvent("pxxy:removeWeaponComponent", weaponName, weaponComponent)
                    self.triggerEvent("pxxy:removeInventoryItem", component.label, false, true)
                end
            end
        end
    end
    ...
    function self.showNotification(msg, notifyType, length)
        self.triggerEvent("pxxy:showNotification", msg, notifyType, length)
    end
    ...
    function self.showAdvancedNotification(sender, subject, msg, textureDict, iconType, flash, saveToBrief, hudColorIndex)
        self.triggerEvent("pxxy:showAdvancedNotification", sender, subject, msg, textureDict, iconType, flash, saveToBrief, hudColorIndex)
    end
    ...
    function self.showHelpNotification(msg, thisFrame, beep, duration)
        self.triggerEvent("pxxy:showHelpNotification", msg, thisFrame, beep, duration)
    end
